  ...
  return (
    <Html center>
      {placeholderSrc ? (
        ...
        <div style={{ color: '#f97316', fontWeight: 700, fontSize: 'clamp(13px, 1.0vw, 15px)' }}>{Math.round(progress)}%</div>
      )}
    </Html>
  );
};
...
const ModelViewer = ({
  url, width = 400, height = 400, modelXOffset = 0, modelYOffset = 0,
  ...
  placeholderSrc, showScreenshotButton = false,
    ...
    <div style={{ width, height, touchAction: 'pan-y pinch-zoom', position: 'relative' }}>
      {showScreenshotButton && (
        <button onClick={capture} style={{
          position: 'absolute', border: '1px solid #fff', right: 16, top: 16,
          zIndex: 10, cursor: 'pointer', padding: '8px 16px', borderRadius: 10
      ...
      >
        ...
        <Suspense fallback={<Loader placeholderSrc={placeholderSrc} />}>
          <ModelInner
            ...
            initYaw={initYaw} initPitch={initPitch} minZoom={minZoomDistance}
            maxZoom={maxZoomDistance} enableMouseParallax={enableMouseParallax}
            enableManualRotation={enableManualRotation} enableHoverRotation={enableHoverRotation}
            enableManualZoom={enableManualZoom} autoFrame={autoFrame} fadeIn={fadeIn}
            autoRotate={autoRotate} autoRotateSpeed={autoRotateSpeed} onLoaded={onModelLoaded}
          />
        </Suspense>
        {!isTouch && (
