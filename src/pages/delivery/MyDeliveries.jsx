    ...
    <DashboardLayout title="My Deliveries">
      ...
      {displayOrders.length === 0 ? (
        <div className="card" style={{ textAlign: 'center', padding: 50 }}>
          <div style={{ fontSize: 'clamp(45px, 1.0vw, 52px)', marginBottom: 12 }}>{tab === 'pending' ? '' : ''}</div>
          ...
          {displayOrders.map(order => (
            <div key={order.id} className="card" style={{ overflow: 'hidden', padding: 0 }}>
              ...
              <div style={{ background: tab === 'pending' ? 'rgba(249,115,22,0.06)' : 'rgba(139,92,246,0.06)', padding: '14px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <span style={{ fontFamily: 'Outfit', fontWeight: 800, fontSize: 'clamp(15px, 1.0vw, 18px)' }}>#{order.id}</span>
                  <span style={{ marginLeft: 8, fontSize: 'clamp(12px, 1.0vw, 14px)', color: 'var(--text-muted)' }}>{new Date(order.orderTime).toLocaleString()}</span>
                </div>
              ...
              <div style={{ padding: 16 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 250px), 1fr))', gap: 12, marginBottom: 14 }}>
                  <div style={{ padding: 12, background: 'var(--bg-tertiary)', borderRadius: 10 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                      <div style={{ fontSize: 'clamp(12px, 1.0vw, 14px)', fontWeight: 800, color: 'var(--text-muted)' }}> DELIVERY LOCATION</div>
                      <a
                    ...
                    <div style={{ fontWeight: 700, fontSize: 'clamp(12px, 1.0vw, 14px)' }}>{order.customerName}</div>
                    <div style={{ fontSize: 'clamp(12px, 1.0vw, 14px)', color: 'var(--text-muted)' }}>{order.customerAddress}</div>
                  </div>
                  <div style={{ padding: 12, background: 'var(--bg-tertiary)', borderRadius: 10 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                      <div style={{ fontSize: 'clamp(12px, 1.0vw, 14px)', fontWeight: 800, color: 'var(--text-muted)' }}> PICKUP FROM</div>
                      <a
                    ...
                    <div style={{ fontWeight: 700, fontSize: 'clamp(12px, 1.0vw, 14px)' }}>{order.restaurantName || 'FitBites Kitchen'}</div>
                    <div style={{ fontSize: 'clamp(12px, 1.0vw, 14px)', color: 'var(--text-muted)' }}>{order.restaurantAddress || 'Koramangala'}</div>
                  </div>
                </div>

                {/* Items */}
                <div style={{ marginBottom: 14 }}>
                  <div style={{ fontSize: 'clamp(12px, 1.0vw, 14px)', fontWeight: 800, color: 'var(--text-muted)', marginBottom: 6 }}> ORDER ITEMS</div>
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    {order.items.map((item, i) => (
                      <span key={i} style={{ padding: '4px 10px', background: 'var(--bg-tertiary)', borderRadius: 8, fontSize: 'clamp(12px, 1.0vw, 14px)', fontWeight: 600 }} className="text-truncate">
                        {item.name} × {item.qty}
                      </span>
                    ))}
                  </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 200px), 1fr))', gap: 8, marginBottom: 14 }}>
                  <div style={{ padding: 10, background: 'rgba(34,197,94,0.06)', borderRadius: 8, textAlign: 'center' }}>
                    <div style={{ fontSize: 'clamp(12px, 1.0vw, 14px)', color: 'var(--text-muted)', fontWeight: 700 }}> TOTAL</div>
                    <div style={{ fontFamily: 'Outfit', fontWeight: 900, fontSize: 'clamp(16px, 1.0vw, 19px)', color: 'var(--accent-green)' }}>₹{order.total}</div>
                  </div>
                  <div style={{ padding: 10, background: 'var(--bg-tertiary)', borderRadius: 8, textAlign: 'center' }}>
                    <div style={{ fontSize: 'clamp(12px, 1.0vw, 14px)', color: 'var(--text-muted)', fontWeight: 700 }}> PAYMENT</div>
                    <div style={{ fontWeight: 800, fontSize: 'clamp(13px, 1.0vw, 15px)' }}>{order.paymentMethod}</div>
                    <div style={{ fontSize: 'clamp(12px, 1.0vw, 14px)', color: order.paymentStatus === 'Paid' ? 'var(--accent-green)' : 'var(--accent-orange)' }}>{order.paymentStatus}</div>
                  </div>
                  <div style={{ padding: 10, background: 'rgba(249,115,22,0.06)', borderRadius: 8, textAlign: 'center' }}>
                    <div style={{ fontSize: 'clamp(12px, 1.0vw, 14px)', color: 'var(--text-muted)', fontWeight: 700 }}> EARN</div>
                    <div style={{ fontFamily: 'Outfit', fontWeight: 900, fontSize: 'clamp(16px, 1.0vw, 19px)', color: 'var(--accent-orange)' }}>₹85</div>
                  </div>
                </div>

                {/* Scheduled dates */}
                {order.scheduledDates?.length > 0 && (
                  <div style={{ padding: '6px 12px', background: 'rgba(34,197,94,0.06)', borderRadius: 8, marginBottom: 14, fontSize: 'clamp(12px, 1.0vw, 14px)' }}>
                ...
                {tab === 'active' && (
                  <div>
                    <div style={{ fontSize: 'clamp(12px, 1.0vw, 14px)', fontWeight: 800, color: 'var(--text-muted)', marginBottom: 6 }}> VERIFY DELIVERY OTP</div>
                    <div style={{ display: 'flex', gap: 8 }}>
                      <input
                        ...
                        maxLength={4}
                        style={{ flex: 1, textAlign: 'center', fontSize: 'clamp(16px, 1.0vw, 19px)', fontWeight: 900, letterSpacing: 8, fontFamily: 'Outfit' }}
                      />
                      <button className="btn btn-success" onClick={() => handleDeliver(order)} disabled={!otpInput[order.id] || otpInput[order.id].length < 4}>
                         Deliver
                      </button>
                    </div>
                    <p style={{ fontSize: 'clamp(12px, 1.0vw, 14px)', color: 'var(--text-muted)', marginTop: 4 }}>Ask customer for OTP to complete delivery</p>
                  </div>
                )}
              </div>
            </div>
