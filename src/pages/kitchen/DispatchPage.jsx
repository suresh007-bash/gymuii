  ...
  return (
    <DashboardLayout title="Dispatch">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 300px), 1fr))', gap: 20 }}>
        ...
        <div>
          {displayOrders.length === 0 ? (
            <div className="card" style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>
              <div style={{ fontSize: 'calc(52px + 0.5vw)', marginBottom: 8 }}>{tab === 'ready' ? '' : tab === 'pending' ? '' : ''}</div>
            ...
            <div key={o.id} className="card" style={{ marginBottom: 12 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                <span style={{ fontWeight: 800, fontFamily: 'Outfit', fontSize: 'calc(19px + 0.5vw)' }}>#{o.id}</span>
              ...
              <div style={{ fontSize: 'calc(17px + 0.5vw)', marginBottom: 4 }}> {o.customerName}</div>
              <div style={{ fontSize: 'calc(16px + 0.5vw)', color: 'var(--text-muted)', marginBottom: 4 }}> {o.customerAddress}</div>
              <div style={{ fontSize: 'calc(16px + 0.5vw)', color: 'var(--text-muted)', marginBottom: 4 }}>️ {o.items.map(i => i.name).join(', ')}</div>
              <div style={{ display: 'flex', gap: 12, fontSize: 'calc(16px + 0.5vw)', marginBottom: 8 }}>
              ...
              {o.driverName && (
                <div style={{ padding: '6px 12px', background: o.status === 'driver_pending' ? 'rgba(249,115,22,0.06)' : 'rgba(34,197,94,0.06)', borderRadius: 8, marginBottom: 8, fontSize: 'calc(16px + 0.5vw)' }}>
        ...
        {/* Drivers Panel */}
        <div>
          <h3 style={{ fontFamily: 'Outfit', fontWeight: 700, marginBottom: 12, fontSize: 'calc(19px + 0.5vw)' }}> Drivers ({drivers.length})</h3>
          {drivers.map(d => {
            ...
            return (
              <div key={d.id} className="card" style={{ marginBottom: 10, padding: 12 }}>
                <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                  <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'linear-gradient(135deg,#f97316,#22c55e)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 'calc(16px + 0.5vw)', fontWeight: 800, color: '#fff' }}>{d.avatar}</div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 700, fontSize: 'calc(17px + 0.5vw)' }}>{d.name}</div>
                    <div style={{ fontSize: 'calc(14px + 0.5vw)', color: 'var(--text-muted)' }}>{d.vehicleType} • {d.licenseNo}</div>
                  </div>
                  <span className={`badge ${activeCount === 0 ? 'badge-green' : 'badge-orange'}`} style={{ fontSize: 'calc(14px + 0.5vw)' }}>
                    {activeCount === 0 ? '🟢 Free' : ` ${activeCount}`}
                  </span>
                </div>
                <div style={{ display: 'flex', gap: 8, marginTop: 6, fontSize: 'calc(14px + 0.5vw)', color: 'var(--text-muted)' }}>
                  <span> {d.rating}</span>
