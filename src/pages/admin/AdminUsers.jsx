...
export default function AdminUsers() {
  ...
  const inp = { width: '100%', padding: '10px 14px', background: 'var(--bg-input)', border: '1px solid var(--border)', borderRadius: 12, color: 'var(--text-primary)', fontSize: 'clamp(15px, 3vw, 20px)' };
  ...
  const handlePromote = () => {
    ...
    setConfirm({
      title: oldRole < newRole ? ' Promote User' : ' Change Role',
      ...
      {confirm && (
        <div className="modal-overlay" onClick={() => setConfirm(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 420, textAlign: 'center' }}>
            ...
            <p style={{ fontSize: 'clamp(14px, 3vw, 19px)', color: 'var(--text-muted)', lineHeight: 1.6, marginBottom: 24 }}>{confirm.msg}</p>
      ...
      {promoteModal && (
        <div className="modal-overlay" onClick={() => setPromoteModal(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 400 }}>
            ...
            <p style={{ fontSize: 'clamp(13px, 3vw, 18px)', color: 'var(--text-muted)', marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 20 }}>
              {roleOptions.map(r => (
                <button key={r} onClick={() => setNewRole(r)} style={{
                  padding: '12px 16px', borderRadius: 10, cursor: 'pointer', fontWeight: 700, fontSize: 'clamp(14px, 3vw, 19px)', textAlign: 'left', transition: 'all 0.2s',
        ...
        <div className="modal-overlay" onClick={() => { setShowAdd(false); setSelRole(''); }}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 500 }}>
            ...
            {!selRole ? (
              /* Role Selection */
              <div>
                <p style={{ fontSize: 'clamp(14px, 3vw, 19px)', color: 'var(--text-muted)', marginBottom: 16 }}>Select User Type:</p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {[{ id: 'owner', icon: '', label: 'Gym Owner', desc: 'Manage gym, trainers, and members' },
                    ...
                    <button key={r.id} onClick={() => setSelRole(r.id)} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 16px', background: 'var(--bg-tertiary)', border: '1px solid var(--border)', borderRadius: 12, cursor: 'pointer', textAlign: 'left', transition: 'all 0.2s', color: 'var(--text-primary)' }}>
                      <span style={{ fontSize: 'clamp(24px, 3vw, 32px)' }}>{r.icon}</span>
                      <div><div style={{ fontWeight: 700, fontSize: 'clamp(15px, 3vw, 20px)' }}>{r.label}</div><div style={{ fontSize: 'clamp(12px, 3vw, 17px)', color: 'var(--text-muted)' }}>{r.desc}</div></div>
                ...
                <tr key={u.id}>
                  <td>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'var(--gradient-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 'clamp(15px, 3vw, 20px)', fontWeight: 800, color: '#fff' }}>{u.avatar}</div>
                      <div style={{ fontWeight: 800, fontSize: 'clamp(15px, 3vw, 21px)' }}>{u.name}</div>
                    </div>
                  </td>
                  <td style={{ fontSize: 'clamp(15px, 3vw, 20px)', fontWeight: 600 }}>{u.email}</td>
                  <td><span className={`badge ${roleBadge[u.role] || 'badge-blue'}`} style={{ fontSize: 'clamp(13px, 3vw, 18px)', padding: '4px 10px' }}>{u.role.toUpperCase()}</span></td>
                  <td style={{ fontSize: 'clamp(15px, 3vw, 20px)', color: 'var(--text-secondary)' }}>{u.gymName || GYMS.find(g => g.id === u.gymId)?.name || u.kitchenName || (u.vehicleType ? `${u.vehicleType} (${u.licenseNo || 'No License'})` : '') || '—'}</td>
                  <td style={{ fontSize: 'clamp(14px, 3vw, 19px)', color: 'var(--text-muted)' }}>{u.joinDate || '—'}</td>
                  <td>
                    <div style={{ display: 'flex', gap: 6 }}>
                      <button className="btn btn-outline btn-sm" style={{ fontSize: 'clamp(14px, 3vw, 19px)' }} title="Change Role" onClick={() => openPromoteModal(u)}> Role</button>
                      <button className="btn btn-outline btn-sm" style={{ color: '#ef4444', fontSize: 'clamp(14px, 3vw, 19px)' }} title="Block User" onClick={() => handleBlock(u)}> Block</button>
                    </div>
                  </td>
                </tr>
              ))}
          ...
          {activeUsers.map(u => (
            <div key={u.id} style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                <div style={{ width: 48, height: 48, borderRadius: '50%', background: 'var(--gradient-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 'clamp(15px, 3vw, 20px)', fontWeight: 800, color: '#fff', flexShrink: 0 }}>{u.avatar}</div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 900, fontSize: 'clamp(15px, 3vw, 21px)', marginBottom: 2 }}>{u.name}</div>
                  <div style={{ fontSize: 'clamp(14px, 3vw, 19px)', color: 'var(--text-muted)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{u.email}</div>
                </div>
                <span className={`badge ${roleBadge[u.role] || 'badge-blue'}`} style={{ flexShrink: 0, fontSize: 'clamp(13px, 3vw, 18px)', padding: '5px 10px', fontWeight: 800 }}>{u.role.toUpperCase()}</span>
              </div>
              <div style={{ display: 'flex', gap: 12, fontSize: 'clamp(14px, 3vw, 19px)', color: 'var(--text-secondary)', flexWrap: 'wrap', fontWeight: 600 }}>
                <span> {u.gymName || GYMS.find(g => g.id === u.gymId)?.name || u.kitchenName || (u.vehicleType ? `${u.vehicleType}` : '—')}</span>
                <span> {u.joinDate || '—'}</span>
              </div>
              <div style={{ display: 'flex', gap: 10 }}>
                <button className="btn btn-outline btn-sm" style={{ flex: 1, fontSize: 'clamp(15px, 3vw, 20px)', padding: '10px' }} onClick={() => openPromoteModal(u)}> Change Role</button>
                <button className="btn btn-outline btn-sm" style={{ flex: 1, fontSize: 'clamp(15px, 3vw, 20px)', padding: '10px', color: '#ef4444', borderColor: 'rgba(239,68,68,0.3)' }} onClick={() => handleBlock(u)}> Block</button>
              </div>
            </div>
          ))}
        </div>
                  ...
                  <tr key={u.id} style={{ opacity: 0.7 }}>
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <div style={{ width: 32, height: 32, borderRadius: '50%', background: '#6b7280', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 'clamp(12px, 3vw, 17px)', fontWeight: 800, color: '#fff' }}>{u.avatar}</div>
                        <div>
                          <div style={{ fontWeight: 700, fontSize: 'clamp(14px, 3vw, 19px)' }}>{u.name}</div>
                          <div style={{ fontSize: 'clamp(12px, 3vw, 16px)', color: '#ef4444', fontWeight: 600 }}>BLOCKED</div>
                        </div>
                      </div>
                    </td>
                    <td style={{ fontSize: 'clamp(13px, 3vw, 18px)' }}>{u.email}</td>
                    <td><span className={`badge ${roleBadge[u.role] || 'badge-blue'}`}>{u.role.toUpperCase()}</span></td>
                    <td style={{ fontSize: 'clamp(13px, 3vw, 18px)', color: 'var(--text-muted)' }}>{u.blockedAt || '—'}</td>
                    <td>
                      <div style={{ display: 'flex', gap: 4 }}>
                        <button className="btn btn-outline btn-sm" style={{ color: '#22c55e' }} title="Unblock" onClick={() => handleUnblock(u)}> Restore</button>
                        <button className="btn btn-outline btn-sm" style={{ color: '#ef4444' }} title="Delete Permanently" onClick={() => handleDeletePermanent(u)}></button>
                      </div>
                    </td>
                  </tr>
                ))}
          ...
          <div className="admin-cards-mobile" style={{ display: 'none' }}>
            {blockedUsers.map(u => (
              <div key={u.id} style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ width: 40, height: 40, borderRadius: '50%', background: '#6b7280', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 'clamp(14px, 3vw, 19px)', fontWeight: 800, color: '#fff', flexShrink: 0 }}>{u.avatar}</div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 800, fontSize: 'clamp(15px, 3vw, 20px)' }}>{u.name}</div>
                    <div style={{ fontSize: 'clamp(12px, 3vw, 16px)', color: '#ef4444', fontWeight: 700 }}> BLOCKED</div>
                  </div>
                  <span className={`badge ${roleBadge[u.role] || 'badge-blue'}`} style={{ flexShrink: 0 }}>{u.role.toUpperCase()}</span>
                </div>
                <div style={{ fontSize: 'clamp(12px, 3vw, 17px)', color: 'var(--text-muted)' }}>
                  {u.email} • Blocked: {u.blockedAt || '—'}
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
                  <button className="btn btn-outline btn-sm" style={{ flex: 1, fontSize: 'clamp(13px, 3vw, 18px)', color: '#22c55e', borderColor: 'rgba(34,197,94,0.3)' }} onClick={() => handleUnblock(u)}> Restore</button>
                  <button className="btn btn-outline btn-sm" style={{ flex: 1, fontSize: 'clamp(13px, 3vw, 18px)', color: '#ef4444', borderColor: 'rgba(239,68,68,0.3)' }} onClick={() => handleDeletePermanent(u)}> Delete</button>
                </div>
              </div>
            ))}
          </div>
