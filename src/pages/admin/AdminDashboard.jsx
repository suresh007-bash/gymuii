...
const MIND_CATS = [
  ...
  { label: 'Orders', img: 'https://images.unsplash.com/photo-1504674900247-0877df9cc836?w=150&q=80' },
...
export default function AdminDashboard() {
  ...
  // Increased from slice(0, 5) to slice(0, 6) to balance out the 6 items in the Roles chart
  ...
  return (
    <DashboardLayout title="Dashboard">
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(80px, 1fr))', gap: 16, paddingBottom: 8 }}>
          {MIND_CATS.map((cat, i) => (
            <Link key={i} to={['/admin/users','/admin/orders','/admin/analytics','/admin/delivery','/admin/settings','/admin/settings','/admin/analytics','/admin/settings'][i]} style={{ textDecoration: 'none', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <div style={{ width: 80, height: 80, borderRadius: '50%', overflow: 'hidden', marginBottom: 6, border: '2px solid #eee' }}>
      ...
      {/* --- RECENT ORDERS & USERS BY ROLE MUTUAL BALANCING GRID --- */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 250px), 1fr))', gap: 20, alignItems: 'stretch' }}>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
            {recentOrders.map(o => (
              <div key={o.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: '1px solid var(--border)' }}>
                ...
                <span className={`badge ${o.status === 'pending' ? 'badge-orange' : o.status === 'preparing' ? 'badge-blue' : o.status === 'delivered' ? 'badge-green' : o.status === 'purple'}`}>{o.status.replace('_', ' ').toUpperCase()}</span>
              </div>
            ))}
          </div>
        </div>
