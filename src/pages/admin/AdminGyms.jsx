    ...
    return (
      <DashboardLayout title="Manage Gyms">
        ...
        <button onClick={() => { setSelectedGym(null); setDetailTab('overview'); }} style={{
          ...
          fontSize: 16, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 6, marginBottom: 16, padding: 0,
        }}>
          ← Back to All Gyms
        </button>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 12, marginBottom: 24 }}>
          {[
            ...
            <div key={i} className="card" style={{ textAlign: 'center', padding: 16 }}>
              ...
              <div style={{ fontSize: 12, color: 'var(--text-muted)', fontWeight: 600, marginTop: 2 }}>{stat.label}</div>
            </div>
          ))}
        </div>
            ...
            {owner && (
              <div className="card">
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 16 }}>
                  ...
                  <div>
                    <div style={{ fontWeight: 800, fontSize: 18 }}>{owner.name}</div>
                    <span className="badge badge-orange" style={{ fontSize: 12 }}>GYM OWNER</span>
                  </div>
                </div>
                    ...
                    return (
                      <div key={trainer.id}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                          <span style={{ fontSize: 15, fontWeight: 700 }}>{trainer.name}</span>
                          <span style={{ fontSize: 14, color: 'var(--text-muted)' }}>{tc.length} clients</span>
                        </div>
                    ...
                    <div key={i} style={{
                      ...
                      <div>
                        <div style={{ fontWeight: 700, fontSize: 16 }}>
                          {plan.name} {plan.popular && <span className="badge badge-orange" style={{ fontSize: 11 }}>POPULAR</span>}
                        </div>
                        <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>{plan.duration}</div>
                      </div>
                      <div style={{ fontSize: 20, fontWeight: 900, color: '#f97316', fontFamily: 'Outfit' }}>₹{plan.price.toLocaleString()}</div>
                    </div>
                  ))}
                </div>
              </div>
                  ...
                  return (
                    <div key={trainer.id} className="card" style={{ overflow: 'hidden' }}>
                      ...
                      <div style={{
                        ...
                        <div>
                          <div style={{ color: '#fff', fontWeight: 800, fontSize: 18 }}>{trainer.name}</div>
                          <div style={{ color: 'rgba(255,255,255,0.7)', fontSize: 14 }}>{trainer.specialization || 'General'}</div>
                        </div>
                      </div>
                      <div style={{ padding: 16 }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 14 }}>
                          <div style={{ textAlign: 'center', padding: 8, background: 'var(--bg-tertiary)', borderRadius: 8 }}>
                            <div style={{ fontSize: 20, fontWeight: 900, color: '#3b82f6' }}>{tc.length}</div>
                            <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>Clients</div>
                          </div>
                          <div style={{ textAlign: 'center', padding: 8, background: 'var(--bg-tertiary)', borderRadius: 8 }}>
                            <div style={{ fontSize: 14, fontWeight: 700 }}>{trainer.certifications || 'N/A'}</div>
                            <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>Certs</div>
                          </div>
                          <div style={{ textAlign: 'center', padding: 8, background: 'var(--bg-tertiary)', borderRadius: 8 }}>
                            <div style={{ fontSize: 14, fontWeight: 700 }}>{trainer.joinDate || 'N/A'}</div>
                        ...
                        {tc.length > 0 && (
                          <div style={{ marginTop: 12, borderTop: '1px solid var(--border)', paddingTop: 12 }}>
                            <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-muted)', marginBottom: 6, textTransform: 'uppercase', letterSpacing: 0.5 }}>Assigned Clients</div>
                    ...
                    <thead>
                      <tr style={{ background: 'var(--bg-tertiary)', borderBottom: '2px solid var(--border)' }}>
                        {['Client', 'Age', 'Gender', 'Goal', 'Trainer', 'Diet', 'Joined'].map(h => (
                          <th key={h} style={{
                            padding: '12px 14px', textAlign: 'left', fontWeight: 800, fontSize: 13,
                      ...
                      {clients.map((client, idx) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td style={{ padding: '10px 14px' }}>
                              <span className={`badge ${client.gender === 'Male' ? 'badge-blue' : 'badge-purple'}`} style={{ fontSize: 12 }}>
                                {client.gender || '—'}
                              </span>
                            </td>
                            <td style={{ padding: '10px 14px' }}>
                              <span className={`badge ${client.goal === 'Weight Loss' ? 'badge-orange' : client.goal === 'Muscle Gain' ? 'badge-green' : 'badge-blue'}`} style={{ fontSize: 12 }}>
                                {client.goal || '—'}
                              </span>
                            </td>
                            <td style={{ padding: '10px 14px' }}>
                            ...
                            <td style={{ padding: '10px 14px', fontSize: 14, color: 'var(--text-muted)' }}>{client.joinDate || '—'}</td>
                          </tr>
                        );
                      })}
                    </tbody>
            ...
            >
              ...
              <div style={{ padding: 16 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 12 }}>
                  <div style={{ textAlign: 'center', padding: 8, background: 'var(--bg-tertiary)', borderRadius: 8 }}>
                    <div style={{ fontSize: 20, fontWeight: 900, color: '#3b82f6' }}>{gymClients.length}</div>
                    <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>Clients</div>
                  </div>
                  <div style={{ textAlign: 'center', padding: 8, background: 'var(--bg-tertiary)', borderRadius: 8 }}>
                    <div style={{ fontSize: 20, fontWeight: 900, color: '#8b5cf6' }}>{gymTrainers.length}</div>
                    <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>Trainers</div>
                  </div>
                  <div style={{ textAlign: 'center', padding: 8, background: 'var(--bg-tertiary)', borderRadius: 8 }}>
                    <div style={{ fontSize: 14, fontWeight: 700 }}>{gymOwner?.name || '—'}</div>
