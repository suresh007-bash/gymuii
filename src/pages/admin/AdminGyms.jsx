    ...
    return (
      <DashboardLayout title="Manage Gyms">
        ...
        <button onClick={() => { setSelectedGym(null); setDetailTab('overview'); }} style={{
          ...
          fontSize: 14, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 6, marginBottom: 16, padding: 0,
        }}>
          ← Back to All Gyms
        </button>
        ...
        <div style={{
          ...
          <div style={{ position: 'absolute', bottom: 20, left: 24 }}>
            ...
            <div style={{ color: 'rgba(255,255,255,0.8)', fontSize: 13, marginTop: 4, display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 12, marginBottom: 24 }}>
          {[
            ...
            <div key={i} className="card" style={{ textAlign: 'center', padding: 16 }}>
              ...
              <div style={{ fontSize: 22, fontWeight: 900, color: stat.color, fontFamily: 'Outfit' }}>{stat.value}</div>
              <div style={{ fontSize: 10, color: 'var(--text-muted)', fontWeight: 600, marginTop: 2 }}>{stat.label}</div>
            </div>
          ))}
        </div>
        ...
        {detailTab === 'overview' && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 320px), 1fr))', gap: 16 }}>
            {/* Gym Info */}
            <div className="card">
              <h4 style={{ fontFamily: 'Outfit', fontWeight: 800, fontSize: 15, marginBottom: 12 }}>📖 About</h4>
              <p style={{ fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.7, marginBottom: 14 }}>{selectedGym.description}</p>
            ...
            {owner && (
              <div className="card">
                <h4 style={{ fontFamily: 'Outfit', fontWeight: 800, fontSize: 15, marginBottom: 12 }}>👑 Owner Details</h4>
            ...
            <div className="card">
              <h4 style={{ fontFamily: 'Outfit', fontWeight: 800, fontSize: 15, marginBottom: 12 }}>📊 Trainer Load Distribution</h4>
              {trainers.length === 0 ? (
                <p style={{ fontSize: 13, color: 'var(--text-muted)' }}>No trainers in this gym.</p>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    ...
                    return (
                      <div key={trainer.id}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                          ...
                          <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>{tc.length} clients</span>
                        </div>
                  ...
                  {unassignedClients > 0 && (
                    <div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                        <span style={{ fontSize: 13, fontWeight: 700, color: '#f59e0b' }}>⚠️ Unassigned</span>
                        <span style={{ fontSize: 12, color: '#f59e0b' }}>{unassignedClients} clients</span>
                      </div>
                      <div style={{ height: 8, borderRadius: 4, background: 'var(--bg-tertiary)', overflow: 'hidden' }}>
                        <div style={{
            ...
            {selectedGym.plans && (
              <div className="card">
                <h4 style={{ fontFamily: 'Outfit', fontWeight: 800, fontSize: 15, marginBottom: 12 }}>💰 Membership Plans</h4>
                    ...
                    <div key={i} style={{
                      ...
                      <div>
                        <div style={{ fontWeight: 700, fontSize: 14 }}>
                          {plan.name} {plan.popular && <span className="badge badge-orange" style={{ fontSize: 9 }}>POPULAR</span>}
                        </div>
                        <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>{plan.duration}</div>
                      </div>
                      <div style={{ fontSize: 18, fontWeight: 900, color: '#f97316', fontFamily: 'Outfit' }}>₹{plan.price.toLocaleString()}</div>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div>
            {trainers.length === 0 ? (
              <div className="card" style={{ textAlign: 'center', padding: 40 }}>
                ...
                <p style={{ color: 'var(--text-muted)', fontSize: 13 }}>This gym has no trainers registered.</p>
              </div>
            ) : (
                  ...
                  return (
                    <div key={trainer.id} className="card" style={{ overflow: 'hidden' }}>
                      ...
                      <div style={{
                        ...
                        <div>
                          <div style={{ color: '#fff', fontWeight: 800, fontSize: 16 }}>{trainer.name}</div>
                          <div style={{ color: 'rgba(255,255,255,0.7)', fontSize: 12 }}>{trainer.specialization || 'General'}</div>
                        </div>
                      </div>
                      <div style={{ padding: 16 }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 14 }}>
                          <div style={{ textAlign: 'center', padding: 8, background: 'var(--bg-tertiary)', borderRadius: 8 }}>
                            <div style={{ fontSize: 18, fontWeight: 900, color: '#3b82f6' }}>{tc.length}</div>
                            <div style={{ fontSize: 10, color: 'var(--text-muted)' }}>Clients</div>
                          </div>
                          <div style={{ textAlign: 'center', padding: 8, background: 'var(--bg-tertiary)', borderRadius: 8 }}>
                            <div style={{ fontSize: 12, fontWeight: 700 }}>{trainer.certifications || 'N/A'}</div>
                            <div style={{ fontSize: 10, color: 'var(--text-muted)' }}>Certs</div>
                          </div>
                          <div style={{ textAlign: 'center', padding: 8, background: 'var(--bg-tertiary)', borderRadius: 8 }}>
                            <div style={{ fontSize: 12, fontWeight: 700 }}>{trainer.joinDate || 'N/A'}</div>
                        ...
                        {tc.length > 0 && (
                          <div style={{ marginTop: 12, borderTop: '1px solid var(--border)', paddingTop: 12 }}>
                            <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', marginBottom: 6, textTransform: 'uppercase', letterSpacing: 0.5 }}>Assigned Clients</div>
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                              {tc.map(c => (
                                <div key={c.id} style={{
                                  ...
                                  <div style={{
                                    width: 20, height: 20, borderRadius: '50%', fontSize: 8, fontWeight: 800,
          ...
          <div>
            {clients.length === 0 ? (
              <div className="card" style={{ textAlign: 'center', padding: 40 }}>
                ...
                <p style={{ color: 'var(--text-muted)', fontSize: 13 }}>This gym has no clients registered.</p>
              </div>
            ) : (<>
              {/* Summary Bar */}
              <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
                <div style={{ padding: '8px 16px', borderRadius: 10, background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.2)', fontSize: 13, fontWeight: 700, color: '#22c55e' }}>
                  ✅ {activeClients} with Trainer
                </div>
                <div style={{ padding: '8px 16px', borderRadius: 10, background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.2)', fontSize: 13, fontWeight: 700, color: '#f59e0b' }}>
                    ...
                    <thead>
                      <tr style={{ background: 'var(--bg-tertiary)', borderBottom: '2px solid var(--border)' }}>
                        {['Client', 'Age', 'Gender', 'Goal', 'Trainer', 'Diet', 'Joined'].map(h => (
                          <th key={h} style={{
                            padding: '12px 14px', textAlign: 'left', fontWeight: 800, fontSize: 11,
                      ...
                      {clients.map((client, idx) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td style={{ padding: '10px 14px' }}>
                              <span className={`badge ${client.gender === 'Male' ? 'badge-blue' : 'badge-purple'}`} style={{ fontSize: 10 }}>
                                {client.gender || '—'}
                              </span>
                            </td>
                            <td style={{ padding: '10px 14px' }}>
                              <span className={`badge ${client.goal === 'Weight Loss' ? 'badge-orange' : client.goal === 'Muscle Gain' ? 'badge-green' : 'badge-blue'}`} style={{ fontSize: 10 }}>
                                {client.goal || '—'}
                              </span>
                            </td>
                            <td style={{ padding: '10px 14px' }}>
                              {trainer ? (
                                ...
                                <span style={{ fontSize: 12, color: '#f59e0b', fontWeight: 600 }}>⚠️ None</span>
                              )}
                            </td>
                            <td style={{ padding: '10px 14px', fontSize: 12, fontWeight: 600 }}>{client.diet || '—'}</td>
                            <td style={{ padding: '10px 14px', fontSize: 12, color: 'var(--text-muted)' }}>{client.joinDate || '—'}</td>
                          </tr>
                        );
                      })}
                    </tbody>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 12, marginBottom: 24 }}>
        {[
          ...
          >
            ...
            <div style={{ fontSize: 26, fontWeight: 900, color: stat.color, fontFamily: 'Outfit' }}>{stat.value}</div>
            ...
            >
              ...
              {gymItem.images && gymItem.images[0] && (
                <div style={{ height: 140, overflow: 'hidden', position: 'relative' }}>
                  ...
                  <div style={{ position: 'absolute', bottom: 10, left: 12, color: '#fff' }}>
                    <div style={{ fontWeight: 800, fontSize: 17, fontFamily: 'Outfit' }}>{gymItem.name}</div>
                    <div style={{ fontSize: 11, opacity: 0.9 }}>📍 {gymItem.location}</div>
                  </div>
                </div>
              )}
              ...
              <div style={{ padding: 16 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 12 }}>
                  <div style={{ textAlign: 'center', padding: 8, background: 'var(--bg-tertiary)', borderRadius: 8 }}>
                    <div style={{ fontSize: 18, fontWeight: 900, color: '#3b82f6' }}>{gymClients.length}</div>
                    <div style={{ fontSize: 10, color: 'var(--text-muted)' }}>Clients</div>
                  </div>
                  <div style={{ textAlign: 'center', padding: 8, background: 'var(--bg-tertiary)', borderRadius: 8 }}>
                    <div style={{ fontSize: 18, fontWeight: 900, color: '#8b5cf6' }}>{gymTrainers.length}</div>
                    <div style={{ fontSize: 10, color: 'var(--text-muted)' }}>Trainers</div>
                  </div>
                  <div style={{ textAlign: 'center', padding: 8, background: 'var(--bg-tertiary)', borderRadius: 8 }}>
                    <div style={{ fontSize: 12, fontWeight: 700 }}>{gymOwner?.name || '—'}</div>
                ...
                <button style={{
                  ...
                  background: 'linear-gradient(135deg, #f97316, #fb923c)', color: '#fff',
                  fontWeight: 700, fontSize: 13, cursor: 'pointer', fontFamily: 'Outfit',
                }}>
