          ...
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 400 }}>
            ...
            <div style={{ textAlign: 'center', margin: '16px 0' }}>
              <div style={{ fontSize: 36, fontWeight: 900, color: 'var(--accent-orange)' }}>
                ₹{showPayment.price}<span style={{ fontSize: 14, color: 'var(--text-muted)' }}>/{showPayment.period}</span>
              </div>
            </div>
        ...
        <div className="card" style={{ marginBottom: 20, border: '2px solid var(--accent-green)', background: 'rgba(34,197,94,0.04)' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontFamily: 'Outfit', fontSize: 28, fontWeight: 900, color: 'var(--accent-green)' }}>
                ₹{currentPlan.price}<span style={{ fontSize: 13, color: 'var(--text-muted)' }}>/mo</span>
              </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 200px), 1fr))', gap: 20 }}>
        {PLANS.map(plan => (
          <div key={plan.id} className="card" style={{ position: 'relative', border: plan.popular ? '2px solid var(--accent-orange)' : undefined }}>
            {plan.popular && (
              <div style={{ position: 'absolute', top: -10, left: '50%', transform: 'translateX(-50%)', background: 'var(--gradient-primary)', color: '#fff', padding: '3px 16px', borderRadius: 20, fontSize: 10, fontWeight: 800 }}>
                MOST POPULAR
              </div>
            )}
            <div style={{ textAlign: 'center', marginBottom: 16 }}>
              ...
              <div>
                <span style={{ fontSize: 32, fontWeight: 900, color: 'var(--accent-orange)' }}>₹{plan.price}</span>
            ...
            <ul style={{ listStyle: 'none', padding: 0, marginBottom: 16 }}>
              {plan.features.map((f, i) => (
                <li key={i} style={{ fontSize: 13, padding: '5px 0', display: 'flex', alignItems: 'center', gap: 8 }}>
