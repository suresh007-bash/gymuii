        ...
        <div className="modal-overlay" onClick={() => setShowPayment(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 400 }}>
            ...
            <div style={{ textAlign: 'center', margin: '16px 0' }}>
              <div style={{ fontSize: 'calc(40px + 0.5vw)', fontWeight: 900, color: 'var(--accent-orange)' }}>
                ₹{showPayment.price}<span style={{ fontSize: 'calc(18px + 0.5vw)', color: 'var(--text-muted)' }}>/{showPayment.period}</span>
              </div>
            </div>
      ...
      {currentPlan && (
        <div className="card" style={{ marginBottom: 20, border: '2px solid var(--accent-green)', background: 'rgba(34,197,94,0.04)' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <div style={{ fontFamily: 'Outfit', fontSize: 'calc(26px + 0.5vw)', fontWeight: 900 }}>{currentPlan.name} Plan</div>
              <div style={{ fontSize: 'calc(17px + 0.5vw)', color: 'var(--text-muted)' }}>{currentPlan.desc}</div>
              
              <div style={{ fontSize: 'calc(16px + 0.5vw)', color: 'var(--text-muted)', marginTop: 4 }}>
            ...
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontFamily: 'Outfit', fontSize: 'calc(32px + 0.5vw)', fontWeight: 900, color: 'var(--accent-green)' }}>
                ₹{currentPlan.price}<span style={{ fontSize: 'calc(17px + 0.5vw)', color: 'var(--text-muted)' }}>/mo</span>
              </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 200px), 1fr))', gap: 20 }}>
        {PLANS.map(plan => (
          <div key={plan.id} className="card" style={{ position: 'relative', border: plan.popular ? '2px solid var(--accent-orange)' : undefined }}>
            {plan.popular && (
              <div style={{ position: 'absolute', top: -10, left: '50%', transform: 'translateX(-50%)', background: 'var(--gradient-primary)', color: '#fff', padding: '3px 16px', borderRadius: 20, fontSize: 'calc(14px + 0.5vw)', fontWeight: 800 }}>
                MOST POPULAR
              </div>
            )}
            <div style={{ textAlign: 'center', marginBottom: 16 }}>
              <h3 style={{ fontFamily: 'Outfit', fontSize: 'calc(24px + 0.5vw)', fontWeight: 900, marginBottom: 4 }}>{plan.name}</h3>
              <div>
                <span style={{ fontSize: 'calc(36px + 0.5vw)', fontWeight: 900, color: 'var(--accent-orange)' }}>₹{plan.price}</span>
                <span style={{ fontSize: 'calc(17px + 0.5vw)', color: 'var(--text-muted)' }}>/{plan.period}</span>
              </div>
            ...
            <ul style={{ listStyle: 'none', padding: 0, marginBottom: 16 }}>
              {plan.features.map((f, i) => (
                <li key={i} style={{ fontSize: 'calc(17px + 0.5vw)', padding: '5px 0', display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 150px), 1fr))', gap: 12 }}>
          {[
            ...
            <div key={i} style={{ textAlign: 'center', padding: 16, background: 'var(--bg-tertiary)', borderRadius: 12 }}>
              <div style={{ fontSize: 'calc(32px + 0.5vw)', marginBottom: 6 }}>
                <Icon icon={b.IconComp} size={28} color="#f97316" />
              </div>
              <div style={{ fontWeight: 700, fontSize: 'calc(17px + 0.5vw)' }}>{b.label}</div>
              <div style={{ fontSize: 'calc(15px + 0.5vw)', color: 'var(--text-muted)' }}>{b.desc}</div>
            </div>
          ))}
        </div>
      </div>
