    ...
    <DashboardLayout title="Subscriptions">
      ...
      {showPayment && (
        <div className="modal-overlay" onClick={() => setShowPayment(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 400 }}>
            <div className="modal-header"><h3 className="modal-title"><Icon icon={CreditCard} size={16} style={{marginRight:6}} /> Subscribe to {showPayment.name}</h3><button className="modal-close" onClick={() => setShowPayment(null)}>✕</button></div>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {['UPI', 'Card', 'Net Banking'].map(p => (<button key={p} className={`btn btn-sm ${payMethod === p ? 'btn-primary' : 'btn-outline'}`} onClick={() => setPayMethod(p)}>{p === 'UPI' ? <Smartphone size={12} style={{marginRight:4}} /> : p === 'Card' ? <CreditCard size={12} style={{marginRight:4}} /> : <Building2 size={12} style={{marginRight:4}} />} {p}</button>))}
              </div>
            </div>
            <button className="btn btn-success btn-lg" style={{ width: '100%' }} onClick={() => subscribe(showPayment)}><Icon icon={CheckCircle2} size={16} style={{marginRight:6}} /> Pay ₹{showPayment.price} & Subscribe</button>
          </div>
        </div>
      )}
      ...
      {currentPlan && (
        <div className="card" style={{ marginBottom: 20, border: '2px solid var(--accent-green)', background: 'rgba(34,197,94,0.04)' }}>
          <div className="card-header"><h3 className="card-title" style={{display:'flex',alignItems:'center',gap:6}}><CheckCircle2 size={16} color="#22c55e" /> Active Subscription</h3><span className="badge badge-green">ACTIVE</span></div>
        ...
        {PLANS.map(plan => (
          <div key={plan.id} className="card" style={{ position: 'relative', border: plan.popular ? '2px solid var(--accent-orange)' : undefined }}>
            ...
            <ul style={{ listStyle: 'none', padding: 0, marginBottom: 16 }}>
              {plan.features.map((f, i) => (<li key={i} style={{ fontSize: 13, padding: '5px 0', display: 'flex', alignItems: 'center', gap: 8 }}><CheckCircle2 size={13} color="#22c55e" /> {f}</li>))}
            </ul>
            {activePlan === plan.id ? (
      ...
      <div className="card" style={{ marginTop: 20 }}>
        <div className="card-header"><h3 className="card-title" style={{display:'flex',alignItems:'center',gap:6}}><Sparkles size={16} color="#f97316" /> Membership Benefits</h3></div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 150px), 1fr))', gap: 12 }}>
          {[{ IconComp: Rocket, label: 'Priority Delivery', desc: 'Get your meals first' }, { IconComp: Banknote, label: '10% Discount', desc: 'On all orders' }, { IconComp: Package, label: 'Free Packs', desc: 'Monthly nutrient pack' }, { IconComp: Target, label: 'AI Suggestions', desc: 'Personalized meals' }].map((b, i) => (
            <div key={i} style={{ textAlign: 'center', padding: 16, background: 'var(--bg-tertiary)', borderRadius: 12 }}>
              <div style={{ fontSize: 28, marginBottom: 6 }}><Icon icon={b.IconComp} size={28} color="#f97316" /></div>
