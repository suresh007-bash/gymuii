    ...
    <DashboardLayout title="Progress Analytics">
      ...
      {tab === 'overview' && (<>
        ...
        <div className="card" style={{ marginBottom: 20 }}>
          <div className="card-header"><h3 className="card-title" style={{display:'flex',alignItems:'center',gap:6}}><Flame size={16} color="#f97316" /> Weekly Calorie Intake</h3><span style={{ fontSize: 'calc(16px + 0.5vw)', color: 'var(--text-muted)' }}>Target: {calTarget} kcal/day</span></div>
            ...
            {last7.map(d => {
              ...
              return (
                <div key={d} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                  <span style={{ fontSize: 'calc(14px + 0.5vw)', fontWeight: 700, marginBottom: 4, color: onTarget ? 'var(--accent-green)' : 'var(--accent-orange)' }}>{cal}</span>
                  ...
                  <span style={{ fontSize: 'calc(14px + 0.5vw)', color: 'var(--text-muted)', marginTop: 4 }}>{d.slice(5)}</span>
                </div>
              );
            })}
          </div>
          {/* Target line label */}
          <div style={{ fontSize: 'calc(15px + 0.5vw)', textAlign: 'center', color: 'var(--text-muted)' }}>🟢 On target (&ge;80%) &nbsp; 🟠 Below target</div>
        </div>

        {/* Protein Chart */}
        <div className="card">
          <div className="card-header"><h3 className="card-title" style={{display:'flex',alignItems:'center',gap:6}}><Beef size={16} color="#22c55e" /> Weekly Protein Intake</h3><span style={{ fontSize: 'calc(16px + 0.5vw)', color: 'var(--text-muted)' }}>Target: {proTarget}g/day</span></div>
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, height: 150, padding: '16px 0' }}>
            {last7.map(d => {
              ...
              return (
                <div key={d} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                  <span style={{ fontSize: 'calc(14px + 0.5vw)', fontWeight: 700, marginBottom: 4 }}>{pro}g</span>
        ...
        <div>
          <div className="card" style={{ marginBottom: 20 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 20, padding: 16 }}>
              <div style={{ width: 80, height: 80, borderRadius: '50%', background: 'var(--gradient-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 'calc(36px + 0.5vw)' }}><Target size={32} color="#fff" /></div>
              <div>
                <div style={{ fontFamily: 'Outfit', fontSize: 'calc(28px + 0.5vw)', fontWeight: 900, marginBottom: 4 }}>{user?.goal || 'Not Set'}</div>
                <div style={{ fontSize: 'calc(17px + 0.5vw)', color: 'var(--text-muted)' }}>Current weight: {user?.weight || '—'}kg • Target calories: {calTarget} kcal/day</div>
                <div style={{ fontSize: 'calc(16px + 0.5vw)', color: 'var(--text-muted)' }}>Diet: {user?.diet || 'Not Set'} • Allergies: {user?.allergies || 'None'}</div>
              </div>
            </div>
          </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 150px), 1fr))', gap: 12 }}>
              {[['Calories', calTarget, 'kcal', '#f97316'], ['Protein', proTarget, 'g', '#22c55e'], ['Carbs', user?.targetCarbs || 250, 'g', '#3b82f6'], ['Fat', user?.targetFat || 70, 'g', '#8b5cf6']].map(([name, val, unit, color]) => (
                <div key={name} style={{ textAlign: 'center', padding: 20, background: 'var(--bg-tertiary)', borderRadius: 16 }}>
                  <div style={{ fontSize: 'calc(32px + 0.5vw)', fontWeight: 900, color, fontFamily: 'Outfit' }}>{val}</div>
                  <div style={{ fontSize: 'calc(16px + 0.5vw)', color: 'var(--text-muted)', fontWeight: 700 }}>{name} ({unit})</div>
                </div>
              ))}
            </div>
          </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 200px), 1fr))', gap: 12 }}>
              {[{ icon: <StatIcon name="calories" />, title: 'First Order', desc: 'Placed your first order', done: totalOrders >= 1 }, { icon: <StatIcon name="protein" />, title: '5 Orders', desc: 'Reached 5 total orders', done: totalOrders >= 5 }, { icon: <StatIcon name="target" />, title: 'On Target', desc: 'Hit calorie target for a day', done: daysOnTarget >= 1 }, { icon: <StatIcon name="calendar" />, title: '7-Day Streak', desc: 'Ordered 7 consecutive days', done: dates.length >= 7 }, { icon: <StatIcon name="dumbbell" />, title: 'Protein King', desc: 'Hit protein target 3 times', done: dates.filter(d => byDate[d].protein >= proTarget).length >= 3 }, { icon: <StatIcon name="star" />, title: 'Loyal Member', desc: 'Spent ₹5000+ total', done: totalSpent >= 5000 }].map((a, i) => (
                <div key={i} style={{ textAlign: 'center', padding: 16, background: a.done ? 'rgba(34,197,94,0.06)' : 'var(--bg-tertiary)', borderRadius: 12, border: a.done ? '1px solid rgba(34,197,94,0.3)' : '1px solid var(--border)', opacity: a.done ? 1 : 0.5 }}>
                  <div style={{ fontSize: 'calc(32px + 0.5vw)', marginBottom: 6 }}>{a.icon}</div>
                  <div style={{ fontWeight: 700, fontSize: 'calc(17px + 0.5vw)' }}>{a.title}</div>
                  <div style={{ fontSize: 'calc(15px + 0.5vw)', color: 'var(--text-muted)' }}>{a.desc}</div>
