      ...
      <DashboardLayout title="Trainer Recommendation">
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '60vh' }}>
          <div className="card" style={{ textAlign: 'center', padding: 'clamp(32px, 2.1vw, 42px)', maxWidth: 480 }}>
            ...
            <h3 style={{ fontWeight: 900, fontSize: 'var(--fs-body)', marginBottom: 8 }}>
              Trainer Required
            </h3>
            <p style={{ color: 'var(--text-muted)', fontSize: 'var(--fs-xs)', lineHeight: 1.7, marginBottom: 24 }}>
    ...
    <DashboardLayout title="Trainer Recommendation">
        ...
        <div className="modal-overlay" onClick={() => setConfirmDelete(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 400 }}>
            ...
            <div style={{ padding: '16px 0', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)', lineHeight: 1.6, marginBottom: 8 }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  {[['morning', ' Breakfast', '8:00 AM'], ['noon', ' Lunch', '12:30 PM'], ['evening', ' Dinner', '7:00 PM']].map(([k, label, time]) => (
                    <button key={k} className={`btn btn-sm ${timing === k ? 'btn-primary' : 'btn-outline'}`} onClick={() => setTiming(k)} style={{ flex: 1 }}>
                      {label}<br /><span style={{ fontSize: 'var(--fs-xs)', opacity: 0.7 }}>{time}</span>
                    </button>
                  ))}
                </div>
              </div>
                ...
                {Object.entries(showOrder.schedule).sort((a, b) => a[0].localeCompare(b[0])).map(([dateStr, slots]) => {
                  ...
                  return (
                    <div key={dateStr} style={{ marginBottom: 10, padding: 10, background: 'var(--bg-tertiary)', borderRadius: 10 }}>
                      <div style={{ fontWeight: 800, fontSize: 'var(--fs-xs)', borderBottom: '1px solid var(--border)', paddingBottom: 4, marginBottom: 6 }}>
                      ...
                      {slots.filter(s => s.items && s.items.length > 0).map((slot, idx) => (
                        <div key={slot.id || idx} style={{ fontSize: 'var(--fs-xs)', marginBottom: 4, paddingLeft: 6 }}>
                          ...
                          <div style={{ color: 'var(--text-muted)', fontSize: 'var(--fs-xs)', paddingLeft: 8 }} className="text-truncate">
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, minmax(40px, 1fr))', gap: 4 }}>
                {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(d => (
                  <div key={d} style={{ textAlign: 'center', fontSize: 'var(--fs-xs)', fontWeight: 800, color: 'var(--text-muted)', padding: 4 }}>{d}</div>
                ))}
      ...
      {myPlans.length > 0 && (
        <div style={{ marginBottom: 24 }}>
          <h3 style={{ fontWeight: 900, fontSize: 'var(--fs-sm)', marginBottom: 14 }}> Plans Assigned to You</h3>
                ...
                <div key={plan.id} className="card" style={{ overflow: 'hidden', position: 'relative' }}>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      cursor: 'pointer', fontSize: 'var(--fs-xs)', transition: 'all 0.2s ease',
                    }}
                  ...
                  <div style={{ background: 'var(--gradient-primary)', padding: '14px 16px', marginBottom: 0 }}>
                    <div style={{ color: '#fff', fontWeight: 800, fontSize: 'var(--fs-xs)' }} className="text-truncate">{plan.name}</div>
                    <div style={{ color: 'rgba(255,255,255,0.8)', fontSize: 'var(--fs-xs)' }}>By: {plan.trainerName || 'Trainer'}</div>
                  </div>
                    ...
                    <div style={{ display: 'flex', gap: 12, marginBottom: 10 }}>
                      {[['', totalCal, 'kcal'], ['', totalPro + 'g', 'protein'], ['₹', totalPrice, 'price']].map(([icon, val, label], i) => (
                        <div key={i} style={{ flex: 1, textAlign: 'center', padding: 8, background: 'var(--bg-tertiary)', borderRadius: 8 }}>
                          <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 900 }}>{icon} {val}</div>
                    ...
                    <div style={{ marginBottom: 10 }}>
                      {items.slice(0, 3).map((item, i) => (
                        <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'center', padding: '4px 0' }}>
                          ...
                          <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 600 }} className="text-truncate">{item.name}</span>
                          <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', marginLeft: 'auto' }}>₹{item.price}</span>
                        </div>
                      ))}
                      {items.length > 3 && <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>+{items.length - 3} more items</div>}
                    </div>
      ...
      {myPlans.length === 0 && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '55vh' }}>
          <div className="card" style={{ textAlign: 'center', padding: 'clamp(28px, 1.7vw, 35px)', maxWidth: 500 }}>
            ...
            <p style={{ color: 'var(--text-muted)', fontSize: 'var(--fs-xs)', lineHeight: 1.7, marginBottom: 20 }}>
            ...
            <div style={{
              ...
              <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 800, color: 'var(--text-secondary)', marginBottom: 10 }}> What to expect:</div>
              {[
                ...
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 0' }}>
                  ...
                  <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)', lineHeight: 1.5 }}>{text}</span>
                </div>
              ))}
            </div>
            ...
            <button
              ...
              style={{ padding: '12px 28px', borderRadius: 12, fontWeight: 700, fontSize: 'var(--fs-xs)' }}
            >
               Browse Menu
            </button>
          </div>
