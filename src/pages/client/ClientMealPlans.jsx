      ...
      {showOrder && (
        <div className="modal-overlay" onClick={() => { setShowOrder(null); setSelectedDates([]); }}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 520 }}>
            ...
            {/* Calendar Grid */}
            <div style={{ marginBottom: 14 }}>
              <label className="form-label">Select Delivery Dates ({selectedDates.length} selected)</label>
                ...
                {calendarDays.map(dateStr => {
                  ...
                  return (
                    <button key={dateStr} onClick={() => toggleDate(dateStr)} style={{
                      padding: '8px 4px', borderRadius: 8, border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 700, transition: 'all 0.2s',
              ...
              return (
                <div key={plan.id} className="card" style={{ overflow: 'hidden' }}>
                  ...
                  <div style={{ padding: 14 }}>
                    ...
                    <button className="btn btn-primary" style={{ width: '100%' }} onClick={() => setShowOrder(plan)}>📅 Schedule & Order</button>
                  </div>
                </div>
              );
            })}
