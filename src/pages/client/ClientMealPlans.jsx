        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '60vh' }}>
          <div className="card" style={{ textAlign: 'center', padding: 'clamp(32px, 6vw, 60px)', maxWidth: 480 }}>
            <div style={{
              ...
              <span style={{ fontSize: 'clamp(33px, 3vw, 44px)' }}></span>
            </div>
            <h3 style={{ fontFamily: 'Outfit', fontWeight: 900, fontSize: 'clamp(18px, 3vw, 24px)', marginBottom: 8 }}>
              Trainer Required
            </h3>
            <p style={{ color: 'var(--text-muted)', fontSize: 'clamp(13px, 3vw, 18px)', lineHeight: 1.7, marginBottom: 24 }}>
            ...
            <button
              ...
              style={{
                ...
                fontWeight: 700, fontSize: 'clamp(13px, 3vw, 18px)', cursor: 'pointer',
    ...
    <DashboardLayout title="Trainer Recommendation">
        ...
        <div className="modal-overlay" onClick={() => setConfirmDelete(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 400 }}>
            ...
            <div style={{ padding: '16px 0', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: 'clamp(13px, 3vw, 18px)', color: 'var(--text-secondary)', lineHeight: 1.6, marginBottom: 8 }}>
                Are you sure you want to delete <strong>"{confirmDelete.name}"</strong>?
              </p>
              <p style={{ fontSize: 'clamp(12px, 3vw, 16px)', color: 'var(--text-muted)' }}>
      ...
      {showOrder && (
        <div className="modal-overlay" onClick={() => { setShowOrder(null); setSelectedDates([]); }}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 520 }}>
            ...
            <p style={{ fontSize: 'clamp(12px, 3vw, 16px)', color: 'var(--text-muted)', marginBottom: 12 }}>
              {showOrder.schedule
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  {[['morning', ' Breakfast', '8:00 AM'], ['noon', ' Lunch', '12:30 PM'], ['evening', ' Dinner', '7:00 PM']].map(([k, label, time]) => (
                    <button key={k} className={`btn btn-sm ${timing === k ? 'btn-primary' : 'btn-outline'}`} onClick={() => setTiming(k)} style={{ flex: 1 }}>
                      {label}<br /><span style={{ fontSize: 'clamp(12px, 3vw, 14px)', opacity: 0.7 }}>{time}</span>
                    </button>
                  ))}
                </div>
              </div>
                ...
                {Object.entries(showOrder.schedule).sort((a, b) => a[0].localeCompare(b[0])).map(([dateStr, slots]) => {
                  ...
                  return (
                    <div key={dateStr} style={{ marginBottom: 10, padding: 10, background: 'var(--bg-tertiary)', borderRadius: 10 }}>
                      <div style={{ fontWeight: 800, fontSize: 'clamp(12px, 3vw, 16px)', borderBottom: '1px solid var(--border)', paddingBottom: 4, marginBottom: 6 }}>
                      ...
                      {slots.filter(s => s.items && s.items.length > 0).map((slot, idx) => (
                        <div key={slot.id || idx} style={{ fontSize: 'clamp(12px, 3vw, 16px)', marginBottom: 4, paddingLeft: 6 }}>
                          ...
                          <div style={{ color: 'var(--text-muted)', fontSize: 'clamp(12px, 3vw, 15px)', paddingLeft: 8 }}>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, minmax(40px, 1fr))', gap: 4 }}>
                {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(d => (
                  <div key={d} style={{ textAlign: 'center', fontSize: 'clamp(12px, 3vw, 14px)', fontWeight: 800, color: 'var(--text-muted)', padding: 4 }}>{d}</div>
                ))}
            ...
            {selectedDates.length > 0 && (
              <div style={{ padding: 10, background: 'rgba(34,197,94,0.06)', borderRadius: 10, marginBottom: 12, fontSize: 'clamp(12px, 3vw, 16px)' }}>
      ...
      {myPlans.length > 0 && (
        <div style={{ marginBottom: 24 }}>
          <h3 style={{ fontFamily: 'Outfit', fontWeight: 900, fontSize: 'clamp(16px, 3vw, 22px)', marginBottom: 14 }}> Plans Assigned to You</h3>
                ...
                <div key={plan.id} className="card" style={{ overflow: 'hidden', position: 'relative' }}>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      cursor: 'pointer', fontSize: 'clamp(13px, 3vw, 18px)', transition: 'all 0.2s ease',
                    }}
                  ...
                  <div style={{ background: 'var(--gradient-primary)', padding: '14px 16px', marginBottom: 0 }}>
                    <div style={{ color: '#fff', fontWeight: 800, fontSize: 'clamp(14px, 3vw, 19px)' }}>{plan.name}</div>
                    <div style={{ color: 'rgba(255,255,255,0.8)', fontSize: 'clamp(12px, 3vw, 15px)' }}>By: {plan.trainerName || 'Trainer'}</div>
                  </div>
                    ...
                    <div style={{ display: 'flex', gap: 12, marginBottom: 10 }}>
                      {[['', totalCal, 'kcal'], ['', totalPro + 'g', 'protein'], ['₹', totalPrice, 'price']].map(([icon, val, label], i) => (
                        <div key={i} style={{ flex: 1, textAlign: 'center', padding: 8, background: 'var(--bg-tertiary)', borderRadius: 8 }}>
                          <div style={{ fontSize: 'clamp(13px, 3vw, 18px)', fontWeight: 900 }}>{icon} {val}</div>
                          <div style={{ fontSize: 'clamp(12px, 3vw, 14px)', color: 'var(--text-muted)' }}>{label}</div>
                        </div>
                      ))}
                    </div>
                    <div style={{ marginBottom: 10 }}>
                      {items.slice(0, 3).map((item, i) => (
                        <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'center', padding: '4px 0' }}>
                          ...
                          <span style={{ fontSize: 'clamp(12px, 3vw, 16px)', fontWeight: 600 }}>{item.name}</span>
                          <span style={{ fontSize: 'clamp(12px, 3vw, 14px)', color: 'var(--text-muted)', marginLeft: 'auto' }}>₹{item.price}</span>
                        </div>
                      ))}
                      {items.length > 3 && <div style={{ fontSize: 'clamp(12px, 3vw, 15px)', color: 'var(--text-muted)' }}>+{items.length - 3} more items</div>}
                    </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '55vh' }}>
          <div className="card" style={{ textAlign: 'center', padding: 'clamp(28px, 5vw, 50px)', maxWidth: 500 }}>
            <div style={{
              ...
              <span style={{ fontSize: 'clamp(36px, 3vw, 48px)' }}></span>
            </div>
            <h3 style={{ fontFamily: 'Outfit', fontWeight: 900, fontSize: 'clamp(18px, 3vw, 24px)', marginBottom: 8 }}>
              Waiting for Trainer Recommendation
            </h3>
            <p style={{ color: 'var(--text-muted)', fontSize: 'clamp(13px, 3vw, 18px)', lineHeight: 1.7, marginBottom: 20 }}>
            ...
            <div style={{
              ...
              <div style={{ fontSize: 'clamp(12px, 3vw, 16px)', fontWeight: 800, color: 'var(--text-secondary)', marginBottom: 10 }}> What to expect:</div>
              {[
                ...
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 0' }}>
                  <span style={{ fontSize: 'clamp(15px, 3vw, 20px)' }}>{icon}</span>
                  <span style={{ fontSize: 'clamp(12px, 3vw, 16px)', color: 'var(--text-secondary)', lineHeight: 1.5 }}>{text}</span>
                </div>
              ))}
            </div>

            <p style={{ fontSize: 'clamp(12px, 3vw, 16px)', color: 'var(--text-muted)', marginBottom: 16 }}>
              In the meantime, you can browse the menu and order your own meals!
            </p>
            <button
              className="btn btn-primary"
              onClick={() => navigate('/client/menu')}
              style={{ padding: '12px 28px', borderRadius: 12, fontWeight: 700, fontSize: 'clamp(13px, 3vw, 18px)' }}
            >
               Browse Menu
            </button>
          </div>
