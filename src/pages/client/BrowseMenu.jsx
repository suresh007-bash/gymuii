    ...
    <div className="ring-container" style={{ textAlign: 'center', position: 'relative', width: size, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      <div style={{ position: 'relative', width: size, height: size, marginBottom: 6 }}>
        ...
        <div style={{
          ...
          <div className="ring-value" style={{ fontWeight: 900, fontSize: 'var(--fs-sm)', color: isOver ? '#ef4444' : color, lineHeight: 1 }}>{displayed}</div>
        </div>
      </div>

      <div className="ring-label" style={{ fontSize: 'var(--fs-xs)', fontWeight: 800, color: 'var(--text-primary)', whiteSpace: 'nowrap' }}>{label}</div>
      <div className="ring-sub" style={{ fontSize: 'var(--fs-xs)', color: isOver ? '#ef4444' : 'var(--text-muted)', marginTop: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', whiteSpace: 'nowrap' }}>
  ...
  return (
    <DashboardLayout title="Home" flush>
      ...
      <style>{`
        ...
        /* ═══ RESPONSIVE MOBILE 1-COLUMN GRID & FIX ALIGNMENT ═══ */
      ...
      {showSchedule && (
        <div className="modal-overlay" onClick={() => setShowSchedule(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 520 }}>
            ...
            <p style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', marginBottom: 16 }}>Select foods and schedule for multiple days</p>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 250px), 1fr))', gap: 6, maxHeight: 200, overflowY: 'auto' }}>
                {menuItems.filter(m => m.available).map(item => (
                  <div key={item.id} onClick={() => toggleSchedItem(item.id)} style={{ padding: 8, background: schedForm.items.includes(item.id) ? 'rgba(249,115,22,0.08)' : 'var(--bg-tertiary)', border: `1px solid ${schedForm.items.includes(item.id) ? 'var(--accent-orange)' : 'var(--border)'}`, borderRadius: 8, cursor: 'pointer', fontSize: 'var(--fs-xs)' }}>
                    ...
                    <div style={{ color: 'var(--text-muted)', fontSize: 'var(--fs-xs)' }}><Flame size={10} style={{ marginRight: 2 }} />{item.calories} • ₹{item.price}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 24, width: '100%', padding: '0 4px', boxSizing: 'border-box' }}>
        ...
        <div style={{
          ...
          padding: '0', color: '#fff', minHeight: 'clamp(202px, 13.8vw, 237px)',
          ...
          <div style={{ flex: '1 1 300px', padding: 'clamp(26px, 1.4vw, 37px) clamp(18px, 1.0vw, 34px)', position: 'relative', zIndex: 2 }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, fontSize: 'var(--fs-xs)', fontWeight: 800, background: 'linear-gradient(135deg, #f97316, #fb923c)', padding: '6px 16px', borderRadius: 24, marginBottom: 16, letterSpacing: 1.2, textTransform: 'uppercase' }}>
              <Leaf size={14} style={{ marginRight: 4 }} /> {bannerCfg?.badge || 'NUTRIENT POWERED'}
            </div>
            <h1 style={{ fontFamily: 'var(--font-heading)', fontSize: 'var(--fs-hero)', fontWeight: 900, lineHeight: 1.1, marginBottom: 16, maxWidth: 420, color: '#fff' }}>
            ...
            <p style={{ fontSize: 'var(--fs-sm)', opacity: 0.8, maxWidth: 380, lineHeight: 1.7, marginBottom: 24, fontWeight: 500, color: '#fff' }}>
            ...
            <div style={{ display: 'flex', gap: 'clamp(14px, 1.0vw, 20px)', flexWrap: 'wrap' }}>
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 'var(--fs-subheading)', fontWeight: 900, background: 'linear-gradient(135deg, #f97316, #fb923c)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>500+</div>
                <div style={{ fontSize: 'var(--fs-xs)', opacity: 0.5, fontWeight: 600, letterSpacing: 0.5 }}>MEALS</div>
              </div>
              <div style={{ width: 1, background: 'rgba(255,255,255,0.1)' }} />
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 'var(--fs-subheading)', fontWeight: 900, background: 'linear-gradient(135deg, #22c55e, #4ade80)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>100%</div>
                <div style={{ fontSize: 'var(--fs-xs)', opacity: 0.5, fontWeight: 600, letterSpacing: 0.5 }}>FRESH</div>
              </div>
              <div style={{ width: 1, background: 'rgba(255,255,255,0.1)' }} />
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 'var(--fs-subheading)', fontWeight: 900, background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>24/7</div>
                <div style={{ fontSize: 'var(--fs-xs)', opacity: 0.5, fontWeight: 600, letterSpacing: 0.5 }}>DELIVERY</div>
              </div>
            </div>
          </div>

          <div style={{ flex: '1 1 280px', maxWidth: 380, position: 'relative', zIndex: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 'clamp(18px, 1.0vw, 21px)', margin: '0 auto' }}>
            <div style={{
              position: 'absolute', width: 'clamp(182px, 13.8vw, 209px)', height: 'clamp(182px, 13.8vw, 209px)', borderRadius: '50%',
            ...
            <div style={{
              width: 'clamp(202px, 13.8vw, 232px)', height: 'clamp(202px, 13.8vw, 232px)', borderRadius: '50%', overflow: 'hidden',
            ...
            <div style={{
              ...
              fontSize: 'var(--fs-xs)', fontWeight: 700, color: '#4ade80', whiteSpace: 'nowrap',
            ...
            <div style={{
              ...
              fontSize: 'var(--fs-xs)', fontWeight: 700, color: '#fb923c', whiteSpace: 'nowrap',
            ...
            <div style={{
              ...
              fontSize: 'var(--fs-xs)', fontWeight: 700, color: '#818cf8', whiteSpace: 'nowrap',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              ...
              <button onClick={() => navigate(`/${user?.role === 'owner' ? 'owner' : user?.role === 'trainer' ? 'trainer' : 'client'}/cart`)} style={{
                ...
                fontSize: 'var(--fs-sm)', fontWeight: 800, display: 'flex', alignItems: 'center', gap: 8,
                ...
                {cartCount > 0 && <span style={{
                  ...
                  background: '#ef4444', color: '#fff', fontSize: 'var(--fs-xs)', fontWeight: 800,
              ...
              <div className="modal-overlay" onClick={() => setShowTargetEditor(false)}>
                <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 500 }}>
                  ...
                  <div style={{ padding: 14, background: 'rgba(139,92,246,0.06)', borderRadius: 12, marginBottom: 16 }}>
                    <div style={{ fontWeight: 800, fontSize: 'var(--fs-xs)', marginBottom: 10, color: '#8b5cf6' }}><Icon icon={BarChart3} size={14} style={{ marginRight: 4 }} /> Auto-Calculate from Body Data</div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 200px), 1fr))', gap: 8, marginBottom: 10 }}>
                      <div>
                        <label style={{ fontSize: 'var(--fs-xs)', fontWeight: 700, color: 'var(--text-muted)' }}>Weight (kg)</label>
                        <input className="form-input" type="number" value={bioForm.weight} onChange={e => setBioForm(p => ({ ...p, weight: Number(e.target.value) }))} style={{ fontSize: 'var(--fs-xs)' }} />
                      </div>
                      <div>
                        <label style={{ fontSize: 'var(--fs-xs)', fontWeight: 700, color: 'var(--text-muted)' }}>Height (cm)</label>
                        <input className="form-input" type="number" value={bioForm.height} onChange={e => setBioForm(p => ({ ...p, height: Number(e.target.value) }))} style={{ fontSize: 'var(--fs-xs)' }} />
                      </div>
                      <div>
                        <label style={{ fontSize: 'var(--fs-xs)', fontWeight: 700, color: 'var(--text-muted)' }}>Age</label>
                        <input className="form-input" type="number" value={bioForm.age} onChange={e => setBioForm(p => ({ ...p, age: Number(e.target.value) }))} style={{ fontSize: 'var(--fs-xs)' }} />
                      </div>
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 200px), 1fr))', gap: 8, marginBottom: 10 }}>
                      <div>
                        <label style={{ fontSize: 'var(--fs-xs)', fontWeight: 700, color: 'var(--text-muted)' }}>Gender</label>
                        <select className="form-select" value={bioForm.gender} onChange={e => setBioForm(p => ({ ...p, gender: e.target.value }))} style={{ fontSize: 'var(--fs-xs)' }}>
                          <option>Male</option><option>Female</option>
                        </select>
                      </div>
                      <div>
                        <label style={{ fontSize: 'var(--fs-xs)', fontWeight: 700, color: 'var(--text-muted)' }}>Goal</label>
                        <select className="form-select" value={bioForm.goal} onChange={e => setBioForm(p => ({ ...p, goal: e.target.value }))} style={{ fontSize: 'var(--fs-xs)' }}>
                          <option>Weight Loss</option><option>Maintenance</option><option>Muscle Gain</option>
                        </select>
                      </div>
                      <div>
                        <label style={{ fontSize: 'var(--fs-xs)', fontWeight: 700, color: 'var(--text-muted)' }}>Activity</label>
                        <select className="form-select" value={bioForm.activityLevel} onChange={e => setBioForm(p => ({ ...p, activityLevel: e.target.value }))} style={{ fontSize: 'var(--fs-xs)' }}>
                  ...
                  <div style={{ marginBottom: 16 }}>
                    <div style={{ fontWeight: 800, fontSize: 'var(--fs-xs)', marginBottom: 10 }}><Icon icon={Edit} size={14} style={{ marginRight: 4 }} /> Manual Targets (or edit calculated values)</div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 250px), 1fr))', gap: 10 }}>
                      {[
                        ...
                        <div key={n.key}>
                          <label style={{ fontSize: 'var(--fs-xs)', fontWeight: 700, color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: 4 }}><Icon icon={n.IconComp} size={12} color={n.color} /> {n.label}</label>
                          <input className="form-input" type="number" value={editTargets[n.key]}
                            onChange={e => setEditTargets(p => ({ ...p, [n.key]: Number(e.target.value) }))}
                            style={{ fontSize: 'var(--fs-sm)', fontWeight: 800, color: n.color }} />
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  {!userHasSetTargets && !hasTargetValues ? (
                    <div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
                        <span style={{ fontSize: 'var(--fs-sm)', fontWeight: 800, display: 'flex', alignItems: 'center', gap: 6 }}><Target size={16} color="#f97316" /> Daily Nutrition</span>
                      </div>
                      ...
                      >
                        <div style={{ fontSize: 'var(--fs-hero)', marginBottom: 10 }}><Target size={40} color="#f97316" /></div>
                        <h4 style={{ fontWeight: 800, fontSize: 'var(--fs-body)', marginBottom: 6 }}>Set Your Daily Target</h4>
                        <p style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', lineHeight: 1.6, marginBottom: 16, maxWidth: 400, margin: '0 auto 16px' }}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'center', gap: 10, flexWrap: 'wrap', marginBottom: 16 }}>
                          {[
                            ...
                            <div key={i} style={{
                              ...
                              fontSize: 'var(--fs-xs)', fontWeight: 700, color: n.color,
                            }}>
                        ...
                        <div style={{
                          ...
                          background: 'linear-gradient(135deg, #f97316, #fb923c)',
                          color: '#fff', fontWeight: 800, fontSize: 'var(--fs-sm)', boxShadow: '0 4px 18px rgba(249,115,22,0.35)',
                        }}>
                    ...
                    <>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
                        ...
                        <button className="btn btn-outline btn-sm" onClick={() => { setEditTargets({ ...targets }); setShowTargetEditor(true); }} style={{ fontSize: 'var(--fs-xs)' }}>
              ...
              {showSuggested && (
                <div style={{ marginBottom: 12 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
                    ...
                    <span style={{ fontWeight: 800, fontSize: 'var(--fs-sm)', color: cfg.color }}>Smart Meal Optimization</span>
                  </div>
                    ...
                    >
                      ...
                      >
                        <div className="food-card-img" style={{ position: 'relative' }}>
                          ...
                          {!item.available && <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#ef4444', fontWeight: 800, fontSize: 'var(--fs-sm)' }}>UNAVAILABLE</div>}
                        </div>
                        <div className="food-card-body">
                          <div className="food-card-name text-truncate">{item.name}</div>
                          <div className="food-card-macros">{item.tags.map(t => <span key={t} className="badge badge-purple" style={{ fontSize: 'var(--fs-sm)', padding: '6px 10px' }}>{t}</span>)}</div>
                          <div style={{ display: 'flex', gap: 6, fontSize: 'var(--fs-sm)', color: 'var(--text-muted)', marginBottom: 6, flexWrap: 'wrap' }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 'auto' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                              ...
                              {item.originalPrice && <>
                                <span style={{ textDecoration: 'line-through', color: 'var(--text-muted)', fontSize: 'var(--fs-sm)' }}>₹{item.originalPrice}</span>
                                <span style={{ color: '#22c55e', fontSize: 'var(--fs-sm)', fontWeight: 800 }}>{item.discount}% off</span>
                              </>}
                            </div>
        ...
        {tab === 'packs' && (
          <div>
            <p style={{ color: 'var(--text-muted)', fontSize: 'var(--fs-xs)', marginBottom: 16 }}>Curated nutrient packs from kitchen, trainers & gym owners. Order a complete meal set!</p>
                ...
                return (
                  <div key={pack.id} className="card" style={{ animation: 'fadeInUp 0.4s ease' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                      <h3 style={{ fontWeight: 800, fontSize: 'var(--fs-sm)' }}><Icon icon={Package} size={16} style={{ marginRight: 6 }} /> {pack.name}</h3>
                    ...
                    <p style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', marginBottom: 10 }}>{pack.description}</p>
                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 10 }}>
                      {items.map(item => (
                        <span key={item.id} style={{ fontSize: 'var(--fs-xs)', padding: '3px 8px', background: item.available ? 'rgba(34,197,94,0.08)' : 'rgba(239,68,68,0.08)', borderRadius: 6, color: item.available ? '#22c55e' : '#ef4444' }}>
                    ...
                    <div style={{ display: 'flex', gap: 16, marginBottom: 12, fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>
                    ...
                    {unavailCount > 0 && <div style={{ background: 'rgba(249,115,22,0.08)', border: '1px solid rgba(249,115,22,0.2)', borderRadius: 8, padding: 8, marginBottom: 10, fontSize: 'var(--fs-xs)', color: '#f97316', display: 'flex', alignItems: 'center', gap: 4 }}><AlertTriangle size={12} /> {unavailCount} item(s) unavailable — you'll be asked to confirm</div>}
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span style={{ fontSize: 'var(--fs-subheading)', fontWeight: 900, color: 'var(--accent-green)' }}>₹{pack.price}</span>
        ...
        {tab === 'plans' && (
          <div>
            <p style={{ color: 'var(--text-muted)', fontSize: 'var(--fs-xs)', marginBottom: 16 }}>Diet plans assigned to you by your trainer. Order directly!</p>
                ...
                {myDietPlans.map(plan => {
                  ...
                  return (
                    <div key={plan.id} className="card">
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                        <h3 style={{ fontWeight: 800, fontSize: 'var(--fs-sm)' }}><Icon icon={ClipboardList} size={16} style={{ marginRight: 6 }} /> {plan.name}</h3>
                        <span className="badge badge-green">{plan.status}</span>
                      </div>
                      <p style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', marginBottom: 10 }}>Created: {new Date(plan.createdAt).toLocaleDateString()}</p>
                      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 12 }}>
                        {planItems.map(item => (
                          <span key={item.id} style={{ fontSize: 'var(--fs-xs)', padding: '4px 10px', background: 'var(--bg-tertiary)', borderRadius: 8 }} className="text-truncate">{item.name} • ₹{item.price}</span>
                        ))}
                      </div>
      ...
      {cart.length > 0 && (
        <div className="sticky-cart-bar">
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div>
              <div style={{ fontWeight: 800, fontSize: 'var(--fs-xs)' }}>{cart.reduce((a, c) => a + c.qty, 0)} items</div>
              <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>₹{cart.reduce((a, c) => a + c.price * c.qty, 0)}</div>
            </div>
          </div>
          <button className="btn btn-primary btn-sm" onClick={() => navigate(`/${user?.role === 'owner' ? 'owner' : user?.role === 'trainer' ? 'trainer' : 'client'}/cart`)} style={{ fontWeight: 800, fontSize: 'var(--fs-xs)', padding: '8px 20px', borderRadius: 10 }}>
            View Cart →
          </button>
        </div>
      )}
