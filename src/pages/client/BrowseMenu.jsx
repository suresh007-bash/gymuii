...
const Ring = ({ value, target, color, size = 90, stroke = 8, icon, label, unit }) => {
  ...
  return (
    <div className="ring-container" style={{ textAlign: 'center', position: 'relative' }}>
      ...
      <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', textAlign: 'center' }}>
        <div className="ring-icon" style={{ fontSize: 16 }}>{icon}</div>
      ...
      <div className="ring-label" style={{ fontSize: 10, fontWeight: 700, color: 'var(--text-muted)', marginTop: 4 }}>{label}</div>
      <div className="ring-sub" style={{ fontSize: 9, color: isOver ? '#ef4444' : 'var(--text-muted)' }}>
...
export default function BrowseMenu() {
  ...
  const saveTargets = () => {
    ...
    // Mark user as having set targets (for first-time setup flow)
    if (!user?.targetsSet) {
  ...
  return (
    <DashboardLayout title="Home" flush>
      ...
      {/* ═══ FOOD MENU TAB ═══ */}
      {tab === 'menu' && (<>
        {/* Sticky Top Section: Search + Nutrition */}
        <div className="nutrition-tracker-sticky">
        {/* Action Bar */}
        <div style={{ display: 'flex', gap: 8, marginBottom: showRings ? 12 : 0, alignItems: 'center' }}>
        ...
        {showRings && (() => {
          ...
          return (
             <div style={{ padding: 12, marginBottom: 0, animation: 'fadeInUp 0.3s ease' }}>
              ...
              {/* Show "Set Target" banner for new users who haven't set targets yet */}
              {!userHasSetTargets && !hasTargetValues ? (
                ...
                /* Normal view with rings for users who have set targets */
                <>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
                    <div>
                      <span style={{ fontFamily: 'Outfit', fontSize: 15, fontWeight: 800, display: 'flex', alignItems: 'center', gap: 6 }}><Target size={16} color="#f97316" /> Daily Nutrition</span>
        ...
        {cat === 'All' && !search && (() => {
          ...
          const goalConfig = {
            'Weight Loss': { label: 'Low-Cal Picks for Weight Loss', LabelIcon: Flame, filter: (m) => m.calories <= 400 && m.protein >= 15, color: '#ef4444', bg: 'rgba(239,68,68,0.06)', border: 'rgba(239,68,68,0.15)' },
            'Muscle Gain': { label: 'High-Protein for Muscle Gain', LabelIcon: Beef, filter: (m) => m.protein >= 35, color: '#22c55e', bg: 'rgba(34,197,94,0.06)', border: 'rgba(34,197,94,0.15)' },
            'Maintenance': { label: 'Balanced Meals for Maintenance', LabelIcon: Target, filter: (m) => m.calories >= 300 && m.calories <= 500, color: '#3b82f6', bg: 'rgba(59,130,246,0.06)', border: 'rgba(59,130,246,0.15)' },
          };
              ...
              return (
                <div key={pack.id} className="card" style={{ animation: 'fadeInUp 0.4s ease' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                    ...
                    <span className={`badge ${pack.creatorRole === 'trainer' ? 'badge-purple' : pack.creatorRole === 'owner' ? 'badge-green' : 'badge-blue'}`}>{pack.creatorRole === 'trainer' ? <><Dumbbell size={10} style={{marginRight:3}} /> Trainer</> : pack.creatorRole === 'owner' ? <><Crown size={10} style={{marginRight:3}} /> Owner</> : <><ChefHat size={10} style={{marginRight:3}} /> Kitchen</>}</span>
                  </div>
