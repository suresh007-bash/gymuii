              ...
              <div key={order.id} className="card" style={{ borderLeft: `4px solid ${statusColor}`, animation: 'fadeInUp 0.4s ease' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10, cursor: 'pointer' }}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ fontFamily: 'Outfit', fontWeight: 900, fontSize: 16, color: 'var(--accent-green)' }}>₹{order.total}</span>
                ...
                {isExpanded && (
                  <div style={{ borderTop: '1px solid var(--border)', paddingTop: 12, marginTop: 4 }}>
                                  ...
                                  <div key={si} style={{ marginBottom: 4 }}>
                                    ...
                                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                                      {(slot.items || []).map((item, i) => (
                                        <span key={i} style={{ fontSize: 11, padding: '3px 8px', background: 'var(--bg-tertiary)', borderRadius: 6 }}>
                                          {item.name} × {item.qty}
                                        </span>
                                      ))}
                                    </div>
                    ...
                    <div style={{ padding: 12, background: 'var(--bg-tertiary)', borderRadius: 10, marginBottom: 10 }}>
                      ...
                      {order.deliveryFee > 0 && <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 4 }}>
                        <span style={{ color: 'var(--text-muted)' }}>Delivery</span><span>₹{order.deliveryFee}</span>
                      </div>}
                      {order.tip > 0 && <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 4 }}>
                        <span style={{ color: 'var(--text-muted)' }}>Tip</span><span>₹{order.tip}</span>
                      </div>}
                      <hr style={{ border: 'none', borderTop: '1px solid var(--border)', margin: '6px 0' }} />
                      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 16, fontWeight: 900, fontFamily: 'Outfit' }}>
                ...
                {!isExpanded && (
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    ...
                    {(order.items || []).length > 4 && <span style={{ fontSize: 11, padding: '3px 8px', color: 'var(--text-muted)' }}>+{order.items.length - 4} more</span>}
                  </div>
                )}
              </div>
            );
