    ...
    <DashboardLayout title="My Orders">
      ...
      {editAddrOrder && (
        <div className="modal-overlay" onClick={() => setEditAddrOrder(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 450 }}>
            ...
            <p style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', marginBottom: 8 }}>Order #{editAddrOrder.id}</p>
            <textarea className="form-input" value={addrText} onChange={e => setAddrText(e.target.value)}
              rows={3} placeholder="Enter delivery address..." style={{ resize: 'vertical', fontSize: 'var(--fs-xs)' }} />
      ...
      <div style={{ display: 'flex', gap: 8, justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          <input type="date" className="form-input" value={filterDate} onChange={e => setFilterDate(e.target.value)}
            style={{ padding: '6px 10px', fontSize: 'var(--fs-xs)', width: 140 }} />
          {filterDate && <button className="btn btn-outline btn-sm" onClick={() => setFilterDate('')} style={{ fontSize: 'var(--fs-xs)' }}> Clear</button>}
        </div>
      </div>
      ...
      {filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: 60 }}>
          <div style={{ fontSize: 'var(--fs-hero)', marginBottom: 12 }}><Package size={48} color="var(--text-muted)" /></div>
          <p style={{ color: 'var(--text-muted)', fontSize: 'var(--fs-xs)' }}>No {tab === 'all' ? '' : tab} orders found</p>
          {filterDate && <p style={{ color: 'var(--text-muted)', fontSize: 'var(--fs-xs)' }}>Try clearing the date filter</p>}
        </div>
      ) : (
              ...
              <div key={order.id} className="card" style={{ borderLeft: `4px solid ${statusColor}`, animation: 'fadeInUp 0.4s ease' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10, cursor: 'pointer' }}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <div style={{ width: 40, height: 40, borderRadius: 10, background: `${statusColor}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 'var(--fs-sm)' }}>
                    ...
                    <div>
                      <div style={{ fontWeight: 800, fontSize: 'var(--fs-xs)' }}>
                        #{formattedId}
                        {hasDates && <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 600, marginLeft: 6, color: '#8b5cf6' }}><Calendar size={11} style={{marginRight:3}} /> {order.scheduledDates.length} dates</span>}
                      </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ fontWeight: 900, fontSize: 'var(--fs-sm)', color: 'var(--accent-green)' }}>₹{order.total}</span>
                    <span className={`badge ${statusBadge[order.status] || 'badge-blue'}`}>{order.status.replace('_', ' ').toUpperCase()}</span>
                    <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', transition: 'transform 0.2s', transform: isExpanded ? 'rotate(180deg)' : 'rotate(0)' }}>▼</span>
                  </div>
                </div>

                {/* Status Message */}
                {['pending', 'preparing', 'ready', 'in_transit', 'driver_pending'].includes(order.status) && (
                  <div style={{ padding: '8px 12px', background: `${statusColor}08`, borderRadius: 8, marginBottom: 10, fontSize: 'var(--fs-xs)', fontWeight: 600, color: statusColor }}>
                        ...
                        <div key={i} style={{ display: 'flex', alignItems: 'center', flex: 1 }}>
                          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', zIndex: 2 }}>
                            <div style={{
                              ...
                              fontSize: 'var(--fs-sm)', fontWeight: 800, flexShrink: 0, transition: 'all 0.3s',
                            ...
                            <span style={{
                              fontSize: 'var(--fs-xs)', fontWeight: 800, marginTop: 4, textAlign: 'center', whiteSpace: 'nowrap',
                ...
                {order.otp && ['pending', 'preparing', 'ready', 'in_transit'].includes(order.status) && (
                  <div style={{ background: 'rgba(249,115,22,0.06)', border: '1px solid rgba(249,115,22,0.15)', borderRadius: 10, padding: '6px 12px', marginBottom: 10, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 600, color: 'var(--text-secondary)' }}><Lock size={11} style={{marginRight:4}} /> Delivery OTP</span>
                    <span style={{ fontSize: 'var(--fs-sm)', fontWeight: 900, color: 'var(--accent-orange)', letterSpacing: 4 }}>{order.otp}</span>
                  </div>
                )}
                ...
                {isExpanded && (
                  <div style={{ borderTop: '1px solid var(--border)', paddingTop: 12, marginTop: 4 }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', background: 'var(--bg-tertiary)', borderRadius: 8, marginBottom: 12 }}>
                      <div>
                        <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', display:'flex', alignItems:'center', gap:4 }}><MapPin size={10} /> Delivery Address</div>
                        <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 600, marginTop: 2 }}>{order.customerAddress || 'No address set'}</div>
                      </div>
                      {isEditable(order) && (
                        <button className="btn btn-outline btn-sm" style={{ fontSize: 'var(--fs-xs)', display:'flex', alignItems:'center', gap:3 }} onClick={() => { setEditAddrOrder(order); setAddrText(order.customerAddress || ''); }}><Edit size={10} /> Edit</button>
                      )}
                    </div>
                    ...
                    {hasDates ? (
                      <div>
                        <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 800, marginBottom: 8, color: '#8b5cf6', display:'flex', alignItems:'center', gap:4 }}><Calendar size={12} /> Date-wise Order Breakdown</div>
                          ...
                          return (
                            <div key={dateStr} style={{
                              ...
                              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                  <span style={{ fontWeight: 800, fontSize: 'var(--fs-xs)' }}>{fmtDate(dateStr)}</span>
                                  {isPast && <span className="badge badge-teal" style={{ fontSize: 'var(--fs-xs)' }}>PAST</span>}
                                </div>
                                {!isPast && isEditable(order) && (
                                  <button className="btn btn-outline btn-sm" style={{ color: 'var(--accent-red)', fontSize: 'var(--fs-xs)', display:'flex', alignItems:'center', gap:3 }}
                              ...
                              {dateData ? (
                                dateData.map((slot, si) => (
                                  <div key={si} style={{ marginBottom: 4 }}>
                                    <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 700, color: 'var(--text-muted)', marginBottom: 2 }}>
                                    ...
                                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                                      {(slot.items || []).map((item, i) => (
                                        <span key={i} style={{ fontSize: 'var(--fs-xs)', padding: '3px 8px', background: 'var(--bg-tertiary)', borderRadius: 6 }} className="text-truncate">
                                          {item.name} × {item.qty}
                                        </span>
                                      ))}
                                    </div>
                      ...
                      <div style={{ marginBottom: 12 }}>
                        <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 800, marginBottom: 8, display:'flex', alignItems:'center', gap:4 }}><Utensils size={12} /> Items Ordered</div>
                        {(order.items || []).map((item, i) => (
                          <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderBottom: '1px solid var(--border)' }}>
                            <div>
                              <span style={{ fontWeight: 700, fontSize: 'var(--fs-xs)' }} className="text-truncate">{item.name}</span>
                              <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', marginLeft: 6 }}>× {item.qty}</span>
                              {item.calories && <span style={{ fontSize: 'var(--fs-xs)', color: '#f97316', marginLeft: 6 }}><Flame size={10} style={{marginRight:2}} />{item.calories * item.qty}</span>}
                            </div>
                            <span style={{ fontWeight: 800, fontSize: 'var(--fs-xs)' }}>₹{item.price || (item.qty * (item.unitPrice || 0))}</span>
                          </div>
                        ))}
                      </div>
                    )}
                    ...
                    <div style={{ padding: 12, background: 'var(--bg-tertiary)', borderRadius: 10, marginBottom: 10 }}>
                      ...
                      {order.deliveryFee > 0 && <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 'var(--fs-xs)', marginBottom: 4 }}>
                        <span style={{ color: 'var(--text-muted)' }}>Delivery</span><span>₹{order.deliveryFee}</span>
                      </div>}
                      {order.tip > 0 && <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 'var(--fs-xs)', marginBottom: 4 }}>
                        <span style={{ color: 'var(--text-muted)' }}>Tip</span><span>₹{order.tip}</span>
                      </div>}
                      <hr style={{ border: 'none', borderTop: '1px solid var(--border)', margin: '6px 0' }} />
                      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 'var(--fs-sm)', fontWeight: 900 }}>
                    ...
                    {/* Driver info */}
                    {order.driverName && (
                      <div style={{ padding: '8px 12px', background: 'rgba(139,92,246,0.05)', borderRadius: 8, marginBottom: 10, fontSize: 'var(--fs-xs)' }}>
                ...
                {!isExpanded && (
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    ...
                    {(order.items || []).length > 4 && <span style={{ fontSize: 'var(--fs-xs)', padding: '3px 8px', color: 'var(--text-muted)' }}>+{order.items.length - 4} more</span>}
                  </div>
                )}
              </div>
            );
