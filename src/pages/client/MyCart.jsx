  ...
  if (orderPlaced) return (
    <DashboardLayout title="My Cart">
      <div style={{ textAlign: 'center', padding: 60 }}>
        <div style={{ fontSize: 'var(--fs-hero)', marginBottom: 16 }}><Sparkles size={72} color="#f97316" /></div>
        ...
        <div style={{ background: 'rgba(249,115,22,0.08)', borderRadius: 16, padding: '16px 32px', display: 'inline-block', marginBottom: 16 }}>
          ...
          <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>Share with delivery person</div>
        </div>
  ...
  return (
    <>
      ...
      <DashboardLayout title="My Cart">
        {cart.length === 0 ? (
        <div style={{ textAlign: 'center', padding: 60 }}>
          <div style={{ fontSize: 'var(--fs-hero)', marginBottom: 16 }}><ShoppingCart size={64} color="var(--text-muted)" /></div>
          <h2 style={{ fontSize: 'var(--fs-subheading)', fontWeight: 800, marginBottom: 8 }}>Your cart is empty</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 320px), 1fr))', gap: 20, alignItems: 'start' }}>
          ...
          <div className="card">
            ...
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', padding: '10px 0', marginBottom: 8, borderBottom: '1px solid var(--border)' }}>
              <span style={{ fontSize: 'var(--fs-xs)', color: '#f97316', fontWeight: 700, display:'flex', alignItems:'center', gap:3 }}><Flame size={14} /> {cartCal} kcal</span>
              <span style={{ fontSize: 'var(--fs-xs)', color: '#22c55e', fontWeight: 700, display:'flex', alignItems:'center', gap:3 }}><Beef size={14} /> {cartPro}g protein</span>
              <span style={{ fontSize: 'var(--fs-xs)', color: '#3b82f6', fontWeight: 700, display:'flex', alignItems:'center', gap:3 }}><Wheat size={14} /> {cartCarb}g carbs</span>
              <span style={{ fontSize: 'var(--fs-xs)', color: '#eab308', fontWeight: 700, display:'flex', alignItems:'center', gap:3 }}><Droplets size={14} /> {cartFat}g fats</span>
              <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>({cart.reduce((a, c) => a + c.qty, 0)} items)</span>
            </div>
            ...
            {cart.map(item => (
              <div key={item.id} style={{ display: 'flex', gap: 12, alignItems: 'center', padding: '12px 0', borderBottom: '1px solid var(--border)', flexWrap: 'wrap' }}>
                ...
                <div style={{ flex: '1 1 200px', minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', margin: '6px 0', lineHeight: 1.5 }}>
                  ...
                  <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 800, color: 'var(--accent-green)' }}>₹{item.price} each</div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginLeft: 'auto' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <button className="btn btn-outline" style={{ width: 32, height: 32, padding: 0, fontSize: 'var(--fs-sm)', display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => { if (item.qty <= 1) remove(item.id); else updateQty(item.id, -1); }}>−</button>
                    <span style={{ fontWeight: 900, fontSize: 'var(--fs-sm)', minWidth: 24, textAlign: 'center' }}>{item.qty}</span>
                    <button className="btn btn-outline" style={{ width: 32, height: 32, padding: 0, fontSize: 'var(--fs-sm)', display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => updateQty(item.id, 1)}>+</button>
                  </div>
                  <span style={{ fontWeight: 900, fontSize: 'var(--fs-xs)', minWidth: 50, textAlign: 'right' }}>₹{item.price * item.qty}</span>
                  <button onClick={() => remove(item.id)} style={{ color: 'var(--accent-red)', fontSize: 'var(--fs-sm)', cursor: 'pointer', background: 'none', border: 'none' }}><Trash2 size={18}/></button>
                </div>
              </div>
            ))}
          </div>
          ...
          <div>
            <div className="card" style={{ marginBottom: 14 }}>
              ...
              <div style={{ marginBottom: 14 }}>
                <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 700, color: 'var(--text-muted)', marginBottom: 6 }}>PAYMENT</div>
              ...
              {/* Tip */}
              <div style={{ marginBottom: 14 }}>
                <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 700, color: 'var(--text-muted)', marginBottom: 6 }}>TIP</div>
                <div style={{ display: 'flex', gap: 6 }}>
              ...
              <div style={{ padding: 14, background: 'var(--bg-tertiary)', borderRadius: 12 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 'var(--fs-xs)', marginBottom: 4 }}><span style={{ color: 'var(--text-muted)' }}>Subtotal</span><span>₹{subtotal}</span></div>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 'var(--fs-xs)', marginBottom: 4 }}><span style={{ color: 'var(--text-muted)' }}>Delivery</span><span>₹{deliveryFee}</span></div>
                {tip > 0 && <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 'var(--fs-xs)', marginBottom: 4 }}><span style={{ color: 'var(--text-muted)' }}>Tip</span><span>₹{tip}</span></div>}
                <hr style={{ border: 'none', borderTop: '1px solid var(--border)', margin: '8px 0' }} />
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 'var(--fs-body)', fontWeight: 900 }}>
                  <span>Total</span><span style={{ color: 'var(--accent-green)' }}>₹{total}</span>
                </div>
              </div>

              <button className="btn btn-success btn-lg" style={{ width: '100%', marginTop: 14, fontSize: 'var(--fs-sm)' }} onClick={handlePlaceOrder}>
                <CheckCircle2 size={16} style={{marginRight:6}} /> Order Now • ₹{total}
              </button>
              <p style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', textAlign: 'center', marginTop: 6 }}>OTP will be generated for delivery verification</p>
            </div>

            {/* Schedule option */}
            <button className="btn btn-outline" style={{ width: '100%', fontSize: 'var(--fs-xs)' }} onClick={() => navigate(`${rolePrefix}/schedule`)}>
