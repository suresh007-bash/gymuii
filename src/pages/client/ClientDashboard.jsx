  ...
  return (
    <DashboardLayout title="Dashboard">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontWeight: 800, fontSize: 'clamp(13px, 3vw, 18px)', color: 'var(--text-primary)' }}>
          <span style={{ color: '#f97316' }}></span> Daily Nutrition
        </div>
        <button style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 8, padding: '6px 12px', fontSize: 'clamp(12px, 3vw, 16px)', fontWeight: 600, cursor: 'pointer', color: 'var(--text-primary)' }}>
           Edit Targets
        </button>
      </div>
            ...
            >
              ...
              <div style={{ position: 'relative', width: 72, height: 72, flexShrink: 0 }}>
                ...
                <div style={{
                  ...
                  <span style={{ fontWeight: 800, fontSize: 'clamp(15px, 3vw, 21px)', color: '#1e293b', fontFamily: 'Outfit' }}>{item.value}</span>
                  <span style={{ fontSize: 'clamp(12px, 3vw, 13px)', color: '#94a3b8', fontWeight: 600 }}>{item.unit}</span>
                </div>
              </div>
            </div>
          ))}
      ...
      <div style={{ 
        ...
        <div className="card" style={{ borderLeft: '4px solid #f97316', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', height: '100%', boxSizing: 'border-box', padding: '24px 20px', minHeight: '190px' }}>
          <div style={{ marginBottom: 12 }}>
            <h4 style={{ fontWeight: 800, fontSize: 'clamp(13px, 3vw, 18px)', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
              <Clock size={14} color="#f97316" /> Breakfast order timing
            </h4>
            <p style={{ fontSize: 'clamp(12px, 3vw, 16px)', color: 'var(--text-muted)', lineHeight: 1.6, margin: 0 }}>
              Order breakfast by 8:00 AM to receive it fresh by 9:00 AM. Best for early training and morning recovery.
            </p>
          </div>
          <span style={{ fontSize: 'clamp(12px, 3vw, 15px)', color: 'var(--accent-orange)', fontWeight: 700, marginTop: 'auto' }}>Recommended order window: 7:45 AM - 8:15 AM</span>
        </div>
        ...
        <div className="card" style={{ borderLeft: '4px solid #22c55e', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', height: '100%', boxSizing: 'border-box', padding: '24px 20px', minHeight: '190px' }}>
          ...
          <span style={{ fontSize: 'clamp(12px, 3vw, 15px)', color: 'var(--accent-green)', fontWeight: 700, marginTop: 'auto' }}>Recommended order window: 12:00 PM - 12:30 PM</span>
        </div>
        ...
        <div className="card" style={{ borderLeft: '4px solid #a855f7', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', height: '100%', boxSizing: 'border-box', padding: '24px 20px', minHeight: '190px' }}>
          ...
          <span style={{ fontSize: 'clamp(12px, 3vw, 15px)', color: '#a855f7', fontWeight: 700, marginTop: 'auto' }}>Recommended order window: 7:00 PM - 7:30 PM</span>
        </div>
      </div>
      ...
      <div style={{ marginBottom: 20 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <h3 style={{ fontFamily: 'Outfit', fontWeight: 900, fontSize: 'clamp(16px, 3vw, 22px)', display: 'flex', alignItems: 'center', gap: 6 }}><Flame size={18} color="#f97316" /> Popular Items</h3>
          <Link to="/client/menu" style={{ color: 'var(--accent-orange)', fontSize: 'clamp(12px, 3vw, 17px)', fontWeight: 700 }}>See all →</Link>
        </div>
            ...
            <Link key={item.id} to="/client/menu" style={{ textDecoration: 'none', flexShrink: 0, width: 'clamp(170px,22vw,220px)' }}>
              <div style={{ borderRadius: 14, overflow: 'hidden', border: '1px solid #eee', background: 'var(--bg-card)', transition: 'transform 0.2s' }}
                ...
                <div style={{ padding: 10 }}>
                  <div style={{ fontWeight: 800, fontSize: 'clamp(12px, 3vw, 16px)', color: 'var(--text-primary)', marginBottom: 2 }}>{item.name}</div>
                  <div style={{ fontSize: 'clamp(12px, 3vw, 14px)', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: 2 }}><Flame size={10} />{item.calories} • ₹{item.price}</div>
                </div>
              </div>
            </Link>
          ))}
