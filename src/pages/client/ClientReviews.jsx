        ...
        <div className="modal-overlay" onClick={() => setShowForm(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 420 }}>
            ...
            <div style={{ textAlign: 'center', marginBottom: 16 }}>
              <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', marginBottom: 8 }}>{showForm.items.map(i => i.name).join(', ')}</div>
              <div style={{ display: 'flex', gap: 8, justifyContent: 'center' }}>
                {[1, 2, 3, 4, 5].map(s => (
                  <button key={s} onClick={() => setRating(s)} style={{ fontSize: 'var(--fs-heading)', background: 'none', border: 'none', cursor: 'pointer', transition: 'transform 0.2s', transform: s <= rating ? 'scale(1.2)' : 'scale(1)', filter: s <= rating ? 'none' : 'grayscale(1) opacity(0.3)' }}></button>
                ))}
              </div>
              <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 700, marginTop: 4, color: rating >= 4 ? 'var(--accent-green)' : rating >= 3 ? 'var(--accent-orange)' : 'var(--accent-red)' }}>{['', 'Poor', 'Below Average', 'Good', 'Very Good', 'Excellent'][rating]}</div>
            </div>
        ...
        <div className="card" style={{ marginBottom: 20 }}>
          ...
          {unreviewed.map(o => (
            <div key={o.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: '1px solid var(--border)' }}>
              <div>
                <div style={{ fontWeight: 700, fontSize: 'var(--fs-xs)' }}>#{o.id} — {o.items.map(i => i.name).join(', ')}</div>
                <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>{new Date(o.orderTime).toLocaleDateString()} • ₹{o.total}</div>
              </div>
          ...
          {reviews.sort((a, b) => new Date(b.date) - new Date(a.date)).map(r => (
            <div key={r.id} style={{ padding: 14, background: 'var(--bg-tertiary)', borderRadius: 12 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                <span style={{ fontWeight: 700, fontSize: 'var(--fs-xs)' }}>Order #{r.orderId}</span>
                <span style={{ color: '#f97316' }}>{''.repeat(r.rating)}</span>
              </div>
              <p style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)', marginBottom: 4 }}>{r.comment}</p>
              <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>{new Date(r.date).toLocaleDateString()}</span>
            </div>
          ))}
        </div>}
      </div>
