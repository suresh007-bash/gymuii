    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
      <div style={{ position: 'relative', width: 120, height: 120, flexShrink: 0 }}>
        ...
        <div style={{
          ...
          <span style={{ fontWeight: 900, fontSize: 'var(--fs-subheading)', color: strokeColor }}>{displayed}</span>
        </div>
      </div>
      <div style={{ textAlign: 'center' }}>
        <div style={{ fontWeight: 800, fontSize: 'var(--fs-sm)', color: '#1e293b', marginBottom: 2 }}>{label}</div>
        <div style={{ fontSize: 'var(--fs-xs)', color: '#94a3b8', fontWeight: 600 }}>{remaining} {unit} left</div>
      </div>
    </div>
  );
}
        ...
        >
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24, paddingBottom: 16, borderBottom: '1px solid #f1f5f9' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <h3 style={{ margin: 0, fontWeight: 800, fontSize: 'var(--fs-sm)', color: '#1e293b' }}>Daily Nutrition</h3>
            </div>
            <button 
              ...
              style={{ 
                ...
                borderRadius: 12, cursor: 'pointer', fontWeight: 700, fontSize: 'var(--fs-xs)', color: '#475569' 
              }}
            >
              ...
              return (
                <div key={m.name} onClick={m.onClick} style={{ background: 'var(--bg-tertiary)', borderRadius: 12, padding: 12, cursor: m.onClick ? 'pointer' : 'default' }} title={`Click for ${m.name} breakdown`}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                    <div style={{ fontWeight: 800, fontSize: 'var(--fs-xs)', display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            {[
              ...
              return (
                <div key={m.name} onClick={m.onClick} style={{
                  ...
                  <div style={{ width: 44, height: 44, borderRadius: 12, background: `${m.color}15`, display: 'flex', alignItems: 'center', justifycenter: 'center', fontSize: 'var(--fs-subheading)' }}>{m.icon}</div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 800, fontSize: 'var(--fs-xs)' }}>{m.name}</div>
                    <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', marginBottom: 4 }}>{m.desc}</div>
                    <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 900 }}>{m.val} <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', fontWeight: 500 }}>/ {m.max}{m.unit}</span></div>
                  </div>
                  <div>
                    {completed ? <span className="badge badge-green" style={{ fontSize: 'var(--fs-xs)', padding: '4px 8px' }}> HIT</span> : <span className="badge badge-blue" style={{ fontSize: 'var(--fs-xs)', padding: '4px 8px' }}> ACTIVE</span>}
                  </div>
                </div>
              );
            })}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 250px), 1fr))', gap: 12 }}>
          {[
            ...
            <div key={name} onClick={onClick} style={{ background: 'var(--bg-tertiary)', borderRadius: 12, padding: 16, cursor: 'pointer' }} title={`Click for ${name} breakdown`}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}><span style={{ fontWeight: 700, fontSize: 'var(--fs-xs)' }} className="text-truncate">{name}</span><span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>{val}/{max}{unit}</span></div>
              ...
              <div style={{ fontSize: 'var(--fs-xs)', color: val > max ? '#ef4444' : val >= max ? color : 'var(--text-muted)', fontWeight: 700, marginTop: 4 }}>
      ...
      <div className="card" style={{ marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {MACROS.map(m => (
              <button key={m.id} onClick={() => setSelectedMacro(m.id)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 16px', borderRadius: 20, fontSize: 'var(--fs-xs)', fontWeight: 800, cursor: 'pointer', border: `1.5px solid ${selectedMacro === m.id ? m.color : 'var(--border)'}`, background: selectedMacro === m.id ? `${m.color}15` : 'var(--bg-secondary)', color: selectedMacro === m.id ? m.color : 'var(--text-muted)' }}>
          ...
          <div>
            ...
            <select className="form-select" value={selectedFoodId} onChange={e => setSelectedFoodId(e.target.value)} style={{ width: '100%', fontSize: 'var(--fs-xs)' }}>
            ...
            {selectedFood && (
              <div style={{ background: 'var(--bg-tertiary)', borderRadius: 16, overflow: 'hidden', border: '1px solid var(--border)' }}>
                <div style={{ position: 'relative', height: 150 }}>
                  ...
                  <div style={{ position: 'absolute', top: 10, left: 10, display: 'flex', gap: 6 }}><span className="badge badge-blue" style={{ fontSize: 'var(--fs-xs)' }}> {selectedFood.prepTime} min</span><span className="badge badge-green" style={{ fontSize: 'var(--fs-xs)' }}> {selectedFood.rating}</span></div>
                  <div style={{ position: 'absolute', bottom: 10, right: 10 }}><span className="badge" style={{ fontSize: 'var(--fs-xs)', fontWeight: 900, padding: '4px 8px', background: activeMacro.color, color: '#fff' }}>{activeMacro.icon} {selectedFood[activeMacro.id]}{activeMacro.unit}</span></div>
                </div>
                <div style={{ padding: 14 }}>
                  <h4 style={{ fontWeight: 900, fontSize: 'var(--fs-xs)', marginBottom: 6 }}>{selectedFood.name}</h4>
                  <p style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', marginBottom: 10 }}>{selectedFood.description || 'Nutrient-rich premium meal.'}</p>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 6, background: 'var(--bg-secondary)', padding: 8, borderRadius: 10 }}>
                    <div style={{ textAlign: 'center' }}><div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>Cal</div><div style={{ fontWeight: 800, color: '#f97316', fontSize: 'var(--fs-xs)' }}> {selectedFood.calories}</div></div>
                    <div style={{ textAlign: 'center', borderLeft: '1px solid var(--border)' }}><div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>Pro</div><div style={{ fontWeight: 800, color: '#22c55e', fontSize: 'var(--fs-xs)' }}> {selectedFood.protein}g</div></div>
                    <div style={{ textAlign: 'center', borderLeft: '1px solid var(--border)' }}><div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>Carbs</div><div style={{ fontWeight: 800, color: '#3b82f6', fontSize: 'var(--fs-xs)' }}> {selectedFood.carbs}g</div></div>
                    <div style={{ textAlign: 'center', borderLeft: '1px solid var(--border)' }}><div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>Fat</div><div style={{ fontWeight: 800, color: '#eab308', fontSize: 'var(--fs-xs)' }}> {selectedFood.fat}g</div></div>
                  </div>
                </div>
              </div>
            )}
            <div style={{ background: 'var(--bg-tertiary)', borderRadius: 16, border: '1px solid var(--border)', padding: 14 }}>
              <h4 style={{ fontWeight: 800, fontSize: 'var(--fs-xs)', marginBottom: 12 }}>{activeMacro.icon} Similar Alternatives</h4>
              {similarFoods.length === 0 ? <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>No macro alternatives found.</div> : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8, maxHeight: 220, overflowY: 'auto' }}>
                  {similarFoods.map(f => (
                    <div key={f.id} style={{ display: 'flex', gap: 10, padding: 6, background: 'var(--bg-secondary)', borderRadius: 10, border: '1px solid var(--border)', alignItems: 'center' }}>
                      ...
                      <div style={{ flex: 1 }}><div style={{ fontWeight: 800, fontSize: 'var(--fs-xs)' }}>{f.name}</div></div>
                      <span className="badge" style={{ fontSize: 'var(--fs-xs)', fontWeight: 800, padding: '3px 6px', background: `${activeMacro.color}15`, color: activeMacro.color }}>{f[activeMacro.id]}{activeMacro.unit}</span>
                    </div>
                  ))}
                </div>
              )}
            ...
            {todayOrders.map(o => (
              <div key={o.id} style={{ padding: 14, background: 'var(--bg-tertiary)', borderRadius: 12 }}>
                {o.items.map((item, idx) => (
                  <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 'var(--fs-xs)', marginBottom: 4 }}>
          ...
          {historyDates.slice(0, 14).map(dateStr => {
            ...
            return (
              <div key={dateStr} style={{ padding: 14, borderRadius: 12, background: 'var(--bg-tertiary)', border: '1px solid var(--border)' }}>
                <div style={{ fontWeight: 800, fontSize: 'var(--fs-xs)', marginBottom: 6 }}>{fmtDate(dateStr)}</div>
        ...
        <div className="modal-overlay" onClick={() => setShowProteinModal(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 540 }}>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 16 }}>
              {MACROS.map(m => (
                <button key={m.id} onClick={() => setSelectedMacro(m.id)} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '6px 12px', borderRadius: 16, fontSize: 'var(--fs-xs)', fontWeight: 800, border: `1.5px solid ${selectedMacro === m.id ? m.color : 'var(--border)'}`, background: selectedMacro === m.id ? `${m.color}15` : 'var(--bg-secondary)', color: selectedMacro === m.id ? m.color : 'var(--text-muted)' }}>
                  <span>{m.icon}</span><span>{m.label}</span>
                </button>
              ))}
            </div>
