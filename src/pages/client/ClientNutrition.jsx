    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
      <div style={{ position: 'relative', width: 120, height: 120, flexShrink: 0 }}>
        ...
        <div style={{
          ...
          <span style={{ fontWeight: 900, fontSize: 26, color: strokeColor, fontFamily: 'Outfit' }}>{displayed}</span>
        </div>
      </div>
      <div style={{ textAlign: 'center' }}>
        <div style={{ fontWeight: 800, fontSize: 16, color: '#1e293b', marginBottom: 2 }}>{label}</div>
        <div style={{ fontSize: 11, color: '#94a3b8', fontWeight: 600 }}>{remaining} {unit} left</div>
      </div>
    </div>
  );
}
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24, paddingBottom: 16, borderBottom: '1px solid #f1f5f9' }}>
            ...
            <button 
              ...
              style={{ 
                ...
                borderRadius: 12, cursor: 'pointer', fontWeight: 700, fontSize: 13, color: '#475569' 
              }}
            >
            ...
            {[
              ...
              return (
                <div key={m.name} onClick={m.onClick} style={{
                  ...
                  <div style={{ width: 44, height: 44, borderRadius: 12, background: `${m.color}15`, display: 'flex', alignItems: 'center', justifycenter: 'center', fontSize: 26 }}>{m.icon}</div>
                  <div style={{ flex: 1 }}>
      ...
      <div className="card" style={{ marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {MACROS.map(m => (
              <button key={m.id} onClick={() => setSelectedMacro(m.id)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 16px', borderRadius: 20, fontSize: 12, fontWeight: 800, cursor: 'pointer', border: `1.5px solid ${selectedMacro === m.id ? m.color : 'var(--border)'}`, background: selectedMacro === m.id ? `${m.color}15` : 'var(--bg-secondary)', color: selectedMacro === m.id ? m.color : 'var(--text-muted)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16 }}>
            ...
            <div style={{ background: 'var(--bg-tertiary)', borderRadius: 16, border: '1px solid var(--border)', padding: 14 }}>
              <h4 style={{ fontFamily: 'Outfit', fontWeight: 800, fontSize: 13, marginBottom: 12 }}>{activeMacro.icon} Similar Alternatives</h4>
              {similarFoods.length === 0 ? <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>No macro alternatives found.</div> : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8, maxHeight: 220, overflowY: 'auto' }}>
                  {similarFoods.map(f => (
                    <div key={f.id} style={{ display: 'flex', gap: 10, padding: 6, background: 'var(--bg-secondary)', borderRadius: 10, border: '1px solid var(--border)', alignItems: 'center' }}>
                      ...
                      <div style={{ flex: 1 }}><div style={{ fontWeight: 800, fontSize: 11 }}>{f.name}</div></div>
                      <span className="badge" style={{ fontSize: 9, fontWeight: 800, padding: '3px 6px', background: `${activeMacro.color}15`, color: activeMacro.color }}>{f[activeMacro.id]}{activeMacro.unit}</span>
                    </div>
                  ))}
                </div>
              )}
        ...
        <div className="modal-overlay" onClick={() => setShowProteinModal(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 540 }}>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 16 }}>
              {MACROS.map(m => (
                <button key={m.id} onClick={() => setSelectedMacro(m.id)} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '6px 12px', borderRadius: 16, fontSize: 11, fontWeight: 800, border: `1.5px solid ${selectedMacro === m.id ? m.color : 'var(--border)'}`, background: selectedMacro === m.id ? `${m.color}15` : 'var(--bg-secondary)', color: selectedMacro === m.id ? m.color : 'var(--text-muted)' }}>
                  <span>{m.icon}</span><span>{m.label}</span>
                </button>
              ))}
            </div>
