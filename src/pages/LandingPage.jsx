  ...
  return (
    <div style={{ fontFamily: "'Outfit', 'Inter', sans-serif", background: '#fff', color: '#1a1a1a', minHeight: '112vh', height: 'auto', overflow: 'visible', position: 'relative' }}>
      ...
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 1000, padding: '16px clamp(16px, 1.4vw, 28px)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(20px)' }}>
        <div style={{ fontSize: 'var(--fs-subheading)', fontWeight: 900, color: '#fff' }}>
          <span style={{ color: '#22c55e' }}>Fit</span>Bites
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 'clamp(12px, 1.0vw, 23px)' }}>
          <Link to="/login" style={{ padding: '10px 24px', background: 'linear-gradient(135deg, #f97316, #fb923c)', color: '#fff', borderRadius: 30, fontWeight: 800, fontSize: 'var(--fs-xs)', textDecoration: 'none' }}>Get Started</Link>
        </div>
      </nav>
      ...
      <section style={{ minHeight: '112vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: 'clamp(100px, 5.2vw, 115px) clamp(16px, 1.4vw, 28px) clamp(40px, 2.1vw, 46px)', position: 'relative', overflow: 'hidden', background: `linear-gradient(rgba(0,0,0,0.55), rgba(0,0,0,0.65)), url(${foodGeneral}) center/cover no-repeat` }}>
        <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 800, color: '#22c55e', letterSpacing: 2, marginBottom: 16 }}>
          <span style={{ color: '#22c55e' }}>Fit</span><span style={{ color: '#fff' }}>Bites</span>
        </div>
        <h1 style={{ fontSize: 'var(--fs-hero)', fontWeight: 900, color: '#fff', lineHeight: 1.1, marginBottom: 20, maxWidth: 700 }}>
          Gym-Powered<br />Food Delivery
        </h1>
        <p style={{ color: 'rgba(255,255,255,0.85)', fontSize: 'var(--fs-sm)', maxWidth: 550, marginBottom: 32, lineHeight: 1.6 }}>
        ...
        <div style={{ display: 'flex', background: '#fff', borderRadius: 50, padding: 6, maxWidth: 520, width: '100%', boxShadow: '0 8px 32px rgba(0,0,0,0.2)' }}>
          <input type="text" placeholder="Search for healthy meals..." style={{ flex: 1, border: 'none', outline: 'none', padding: '12px 16px', fontSize: 'var(--fs-xs)', borderRadius: 50, color: '#333', minWidth: 0 }} />
          <Link to="/login" style={{ padding: '12px 24px', background: '#ef4444', color: '#fff', borderRadius: 50, fontWeight: 800, fontSize: 'var(--fs-xs)', textDecoration: 'none', display: 'flex', alignItems: 'center', whiteSpace: 'nowrap' }}>Search</Link>
        </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'clamp(20px, 1.4vw, 35px)', marginTop: 48, justifyContent: 'center' }}>
          {STATS.map((s, i) => (
            <div key={i} style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 'var(--fs-subheading)', fontWeight: 900, color: '#fff' }}>{s.val}</div>
              <div style={{ fontSize: 'var(--fs-xs)', color: 'rgba(255,255,255,0.7)', fontWeight: 600 }}>{s.label}</div>
            </div>
          ))}
        </div>

        <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: 'var(--fs-xs)', marginTop: 40 }}>Scroll down ▼</div>
      </section>
      ...
      <section style={{ padding: '40px 0', background: '#fff' }}>
        ...
        >
          <div style={{ maxWidth: 700, margin: '0 auto 24px', textAlign: 'center' }}>
            <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 800, color: '#f97316', letterSpacing: 2, marginBottom: 8 }}>
              WHY FITBITES
            </div>
            <h2 style={{ fontSize: 'var(--fs-heading)', fontWeight: 900, marginBottom: 16 }}>
          ...
          <div className="landing-cards-grid">
            {FEATURES.slice(0, 4).map((f, i) => (
              ...
              >
                ...
                <p style={{ color: '#888', fontSize: 'var(--fs-sm)', lineHeight: 1.6 }}>{f.desc}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="landing-cards-grid">
            {[
              ...
              >
                ...
                <h3 style={{ fontSize: 'var(--fs-body)', fontWeight: 800, marginBottom: 10 }}>
                  {s.title}
                </h3>

                <p style={{ fontSize: 'var(--fs-sm)', color: '#888', lineHeight: 1.6 }}>
                  {s.desc}
                </p>
              </div>
            ))}
      ...
      <section style={{ padding: '40px 0', background: '#fff' }}>
        ...
        >
          <div style={{ maxWidth: 700, margin: '0 auto 24px', textAlign: 'center' }}>
            <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 800, color: '#22c55e', letterSpacing: 2, marginBottom: 8 }}>
              FOR GYM OWNERS
            </div>
            <h2 style={{ fontSize: 'var(--fs-heading)', fontWeight: 900, marginBottom: 16 }}>
          ...
          <div className="landing-cards-grid">
            {[
              ...
              >
                <div style={{ fontSize: 'var(--fs-subheading)', marginBottom: 8 }}>{s.icon}</div>
                ...
                <div style={{ color: '#888', fontSize: 'var(--fs-xs)', fontWeight: 600 }}>
                  {s.label}
                </div>
              </div>
            ))}
      ...
      <section id="menu" style={{ padding: 'clamp(20px, 1.4vw, 28px) 0', background: '#fafafa' }}>
        <div style={{ width: '100%', padding: '0 clamp(16px, 1.4vw, 28px)', boxSizing: 'border-box' }}>
          <div style={{ textAlign: 'center', marginBottom: 48 }}>
            <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 800, color: '#22c55e', letterSpacing: 2, marginBottom: 8 }}>OUR MENU</div>
            <h2 style={{ fontSize: 'var(--fs-heading)', fontWeight: 900, marginBottom: 12 }}>Fresh, Healthy & Delicious</h2>
          </div>
            ...
            {MENU_ITEMS.map(item => (
              <div key={item.id} className="landing-menu-card" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
                ...
                <div style={{ padding: 14, display: 'flex', flexDirection: 'column', flex: 1 }}>
                  <div style={{ fontWeight: 800, fontSize: 'var(--fs-body)', marginBottom: 6, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }} className="text-truncate">{item.name}</div>
                  <div style={{ fontSize: 'var(--fs-xs)', color: '#888', marginBottom: 10 }}> {item.calories} kcal • {item.protein}g Protein</div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 'auto' }}>
                    <span style={{ fontWeight: 900, color: '#22c55e', fontSize: 'var(--fs-body)' }}>₹{item.price}</span>
                    <span style={{ fontSize: 'var(--fs-sm)', color: '#f97316', fontWeight: 700 }}> {item.rating}</span>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div style={{ textAlign: 'center', marginTop: 32 }}>
            <Link to="/login" style={{ padding: '14px 36px', background: 'linear-gradient(135deg, #f97316, #fb923c)', color: '#fff', borderRadius: 50, fontWeight: 800, fontSize: 'var(--fs-xs)', textDecoration: 'none', display: 'inline-block' }}>View Full Menu →</Link>
          </div>
        </div>
      </section>
      ...
      >
        ...
        >
          <div style={{ textAlign: 'center', marginBottom: 48 }}>
            ...
            <h2 style={{ fontSize: 'var(--fs-heading)', fontWeight: 900 }}>
              Subscription Plans
            </h2>
          </div>
      ...
      <section style={{ padding: 'clamp(20px, 1.4vw, 28px) clamp(16px, 1.4vw, 28px)', background: '#fafafa', boxSizing: 'border-box' }}>
        <div style={{ maxWidth: 1000, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 48 }}>
            <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 800, color: '#f97316', letterSpacing: 2, marginBottom: 8 }}>TESTIMONIALS</div>
            <h2 style={{ fontSize: 'var(--fs-heading)', fontWeight: 900 }}>Loved by Fitness Community</h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 340px), 1fr))', gap: 24 }}>
            {TESTIMONIALS.map((t, i) => (
              <div key={i} style={{ background: '#fff', borderRadius: 16, padding: 24, border: '1px solid #eee' }}>
                <div style={{ fontSize: 'var(--fs-sm)', color: '#555', lineHeight: 1.7, marginBottom: 16, fontWeight: 500 }}>"{t.text}"</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'linear-gradient(135deg, #f97316, #22c55e)', display: 'flex', alignItems: 'center', justifycontent: 'center', color: '#fff', fontWeight: 900, fontSize: 'var(--fs-xs)' }}>{t.name.split(' ').map(w => w[0]).join('')}</div>
                  <div><div style={{ fontWeight: 800, fontSize: 'var(--fs-xs)' }} className="text-truncate">{t.name}</div><div style={{ fontSize: 'var(--fs-xs)', color: '#888' }}>{t.role}</div></div>
      ...
      <section style={{ padding: 'clamp(20px, 1.4vw, 28px) clamp(16px, 1.4vw, 28px)', background: 'linear-gradient(135deg, #f97316, #22c55e)', textAlign: 'center', boxSizing: 'border-box' }}>
        <h2 style={{ fontSize: 'var(--fs-heading)', fontWeight: 900, color: '#fff', marginBottom: 12 }}>Ready to Eat Healthy?</h2>
        <p style={{ color: 'rgba(255,255,255,0.9)', fontSize: 'var(--fs-sm)', marginBottom: 28 }}>Join thousands of gym members who fuel their fitness with FitBites</p>
        <Link to="/login" style={{ padding: '16px 40px', background: '#fff', color: '#f97316', borderRadius: 50, fontWeight: 900, fontSize: 'var(--fs-sm)', textDecoration: 'none', display: 'inline-block', boxShadow: '0 8px 32px rgba(0,0,0,0.2)' }}>Get Started — It's Free</Link>
      </section>

      {/* ═══ FOOTER ═══ */}
      <footer style={{ padding: '40px 16px', background: '#111', color: 'rgba(255,255,255,0.6)', textAlign: 'center', boxSizing: 'border-box' }}>
        <div style={{ fontSize: 'var(--fs-body)', fontWeight: 900, color: '#fff', marginBottom: 12 }}><span style={{ color: '#22c55e' }}>Fit</span>Bites</div>
        <p style={{ fontSize: 'var(--fs-xs)', marginBottom: 16 }}>Gym-Powered Food Delivery • Macro-Tracked • Nutritionist-Crafted</p>
        <div style={{ display: 'flex', justifyContent: 'center', gap: 24, marginBottom: 16 }}>
          {['About', 'Contact', 'Privacy'].map(l => (<a key={l} href="#" style={{ color: 'rgba(255,255,255,0.5)', textDecoration: 'none', fontSize: 'var(--fs-xs)' }}>{l}</a>))}
        </div>
        <p style={{ fontSize: 'var(--fs-xs)', color: 'rgba(255,255,255,0.3)' }}> 2026 FitBites. All rights reserved.</p>
      </footer>
    </div>
  );
}