  ...
  return (
    <DashboardLayout title="Dashboard">
      ...
      <div style={{ borderRadius: 20, overflow: 'hidden', marginBottom: 20, background: `linear-gradient(rgba(0,0,0,0.45), rgba(0,0,0,0.55)), url(${trainerWorkout}) center/cover`, padding: '48px 40px', color: '#fff' }}>
        <div style={{ fontSize: 'clamp(12px, 1.0vw, 14px)', fontWeight: 800, background: '#22c55e', display: 'inline-block', padding: '4px 14px', borderRadius: 20, marginBottom: 12, letterSpacing: 1 }}><Crown size={14} style={{marginRight:4}} /> GYM OWNER</div>
        <h1 style={{ fontFamily: 'Outfit', fontSize: 'clamp(28px, 1.4vw, 33px)', fontWeight: 900, lineHeight: 1.15, marginBottom: 12, maxWidth: 450 }}>{user?.gymName}</h1>
        <p style={{ fontSize: 'clamp(13px, 1.0vw, 15px)', opacity: 0.85 }}><MapPin size={14} style={{marginRight:2}} /> {user?.gymLocation} • {members.length} members • {trainers.length} trainers • ₹{memberRevenue.toLocaleString()} revenue</p>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 250px), 1fr))', gap: 16, marginBottom: 20 }}>
        <div className="card" style={{ borderLeft: '4px solid #f97316' }}><h4 style={{ fontWeight: 800, fontSize: 'clamp(13px, 1.0vw, 15px)', marginBottom: 6 }}><Users size={14} style={{marginRight:4}} /> Member Management</h4><p style={{ fontSize: 'clamp(12px, 1.0vw, 14px)', color: 'var(--text-muted)', lineHeight: 1.6, marginBottom: 6 }}>Manage your gym members, trainers, and create nutrient packs for clients.</p><Link to="/owner/members" style={{ fontSize: 'clamp(12px, 1.0vw, 14px)', color: 'var(--accent-orange)', fontWeight: 700 }}>Manage Members →</Link></div>
        <div className="card" style={{ borderLeft: '4px solid #22c55e' }}><h4 style={{ fontWeight: 800, fontSize: 'clamp(13px, 1.0vw, 15px)', marginBottom: 6 }}><Package size={14} style={{marginRight:4}} /> Nutrient Packs</h4><p style={{ fontSize: 'clamp(12px, 1.0vw, 14px)', color: 'var(--text-muted)', lineHeight: 1.6, marginBottom: 6 }}>Create curated meal packs and send to clients for easy ordering.</p><button className="btn btn-sm btn-outline" onClick={() => setShowPack(true)}>Create Pack →</button></div>
      </div>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 16, overflowX: 'auto', paddingBottom: 8 }}>{MIND_CATS.map((cat, i) => {
          ...
          return (<Link key={i} to={paths[i] || '/owner/dashboard'} style={{ textDecoration: 'none', textAlign: 'center', flexShrink: 0 }}><div style={{ width: 80, height: 80, borderRadius: '50%', overflow: 'hidden', marginBottom: 6, border: '2px solid #eee' }}><img src={cat.img} alt={cat.label} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /></div><div style={{ fontSize: 'clamp(12px, 1.0vw, 14px)', fontWeight: 700, color: 'var(--text-primary)' }}>{cat.label}</div></Link>);
        })}</div>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 250px), 1fr))', gap: 20 }}>
        <div className="card"><div className="card-header"><h3 className="card-title"><User size={14} style={{marginRight:4}} /> Direct Clients ({directClients.length})</h3></div>{directClients.length === 0 ? <p style={{ color: 'var(--text-muted)', padding: 12, fontSize: 'clamp(12px, 1.0vw, 14px)' }}>All clients assigned to trainers</p> : directClients.slice(0,5).map(c => (<div key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: '1px solid var(--border)' }}><div style={{ width: 32, height: 32, borderRadius: '50%', background: 'var(--gradient-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 'clamp(12px, 1.0vw, 14px)', fontWeight: 800, color: '#fff' }}>{c.avatar}</div><div style={{ flex: 1 }}><div style={{ fontWeight: 700, fontSize: 'clamp(12px, 1.0vw, 14px)' }} className="text-truncate">{c.name}</div><div style={{ fontSize: 'clamp(12px, 1.0vw, 14px)', color: 'var(--text-muted)' }}>{c.goal}</div></div></div>))}</div>
        <div className="card"><div className="card-header"><h3 className="card-title"><Dumbbell size={14} style={{marginRight:4}} /> Trainers ({trainers.length})</h3></div>{trainers.map(t => (<div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 0', borderBottom: '1px solid var(--border)' }}><div style={{ width: 32, height: 32, borderRadius: '50%', background: 'linear-gradient(135deg, #22c55e, #4ade80)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 'clamp(12px, 1.0vw, 14px)', fontWeight: 800, color: '#fff' }}>{t.avatar}</div><div style={{ flex: 1 }}><div style={{ fontWeight: 700, fontSize: 'clamp(12px, 1.0vw, 14px)' }} className="text-truncate">{t.name}</div><div style={{ fontSize: 'clamp(12px, 1.0vw, 14px)', color: 'var(--text-muted)' }}>{t.specialization}</div></div><span className="badge badge-green">{members.filter(m => m.trainerId === t.id).length} clients</span></div>))}</div>
      </div>
    </DashboardLayout>
  );
}
