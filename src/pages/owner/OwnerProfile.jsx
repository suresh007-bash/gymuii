    ...
    <DashboardLayout title="Owner Profile">
      <div style={{ maxWidth: 600, margin: '0 auto' }}>
        <div className="card" style={{ textAlign: 'center', padding: 32, marginBottom: 20 }}>
          <div style={{ width: 72, height: 72, borderRadius: '50%', background: 'linear-gradient(135deg, #22c55e, #4ade80)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 'clamp(21px, 3vw, 28px)', fontWeight: 900, color: '#fff', margin: '0 auto 12px' }}>{user?.avatar}</div>
          <h2 style={{ fontFamily: 'Outfit', fontWeight: 800, marginBottom: 4 }}>{user?.name}</h2>
          <p style={{ color: 'var(--accent-green)', fontSize: 'clamp(12px, 3vw, 17px)', fontWeight: 700 }}>{user?.gymName}</p>
          <p style={{ color: 'var(--text-muted)', fontSize: 'clamp(12px, 3vw, 16px)' }}><MapPin size={14} style={{marginRight:2}} /> {user?.gymLocation}</p>
        </div>
