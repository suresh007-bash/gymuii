  ...
  return (
    <DashboardLayout title="Member Management">
      ...
      <div className="card"><table className="data-table"><thead><tr><th>Member</th><th>Goal</th><th>Trainer</th><th>Diet</th><th>Joined</th><th>Actions</th></tr></thead>
        <tbody>{shown.map(c=>(
          <tr key={c.id}>
            <td><div style={{display:'flex',alignItems:'center',gap:10}}><div style={{width:32,height:32,borderRadius:'50%',background:'var(--gradient-primary)',display:'flex',alignItems:'center',justifyContent:'center',fontSize: 'var(--fs-xs)',fontWeight:800,color:'#fff'}}>{c.avatar}</div><div><div style={{fontWeight:700,fontSize: 'var(--fs-xs)'}} className="text-truncate">{c.name}</div><div style={{fontSize: 'var(--fs-xs)',color:'var(--text-muted)'}}>{c.email}</div></div></div></td>
            ...
            <td style={{fontSize: 'var(--fs-xs)'}}>{c.diet}</td>
