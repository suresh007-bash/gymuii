      ...
      {confirm && (
        <div className="modal-overlay" style={{ zIndex: 1100 }} onClick={() => setConfirm(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 420, textAlign: 'center' }}>
            <div style={{ fontSize: 48, marginBottom: 12 }}><AlertTriangle size={48} color="#f59e0b" /></div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
        <button
          ...
          style={{
            ...
            fontWeight: 700, fontSize: 14, fontFamily: 'Outfit', transition: 'all 0.3s',
      ...
      {tab === 'trainers' && (<>
        ...
        {trainers.length === 0 && (
          <div className="card" style={{ textAlign: 'center', padding: 40 }}>
            <div style={{ fontSize: 48, marginBottom: 12 }}><Dumbbell size={48} color="#8b5cf6" /></div>
            <h3 style={{ fontWeight: 800, marginBottom: 8 }}>No Trainers Yet</h3>
            <p style={{ color: 'var(--text-muted)', fontSize: 13 }}>Add trainers or wait for join requests from trainers.</p>
          </div>
        )}
      </>)}
      ...
      {tab === 'requests' && (
        <div style={{ maxWidth: 700, margin: '0 auto' }}>
          ...
          <div style={{ marginBottom: 28 }}>
            <h3 style={{ fontFamily: 'Outfit', fontWeight: 800, fontSize: 18, marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            {pendingGymRequests.length === 0 && processedGymRequests.length === 0 ? (
              <div className="card" style={{ textAlign: 'center', padding: 32 }}>
                <div style={{ fontSize: 36, marginBottom: 8 }}><Inbox size={36} color="var(--text-muted)" /></div>
                <p style={{ color: 'var(--text-muted)', fontSize: 13 }}>No trainer join requests yet. Trainers can find your gym and send requests to join.</p>
              </div>
            ) : (
                ...
                {pendingGymRequests.map(req => (
                  <div key={req.id} className="card" style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ fontWeight: 800, fontSize: 15 }}>{req.trainerName}</div>
                        <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>{req.trainerSpecialization || 'General Trainer'}</div>
                ...
                {processedGymRequests.map(req => (
                  <div key={req.id} className="card" style={{ padding: 14, opacity: 0.7 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ fontWeight: 700, fontSize: 14 }}>{req.trainerName}</div>
          ...
          <div>
            ...
            {pendingClientRequests.length === 0 && processedClientRequests.length === 0 ? (
              <div className="card" style={{ textAlign: 'center', padding: 32 }}>
                <div style={{ fontSize: 36, marginBottom: 8 }}><Inbox size={36} color="var(--text-muted)" /></div>
                <p style={{ color: 'var(--text-muted)', fontSize: 13 }}>No client trainer requests yet. Clients can request trainers from the "Join Gym" page.</p>
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  return (
                    <div key={req.id} className="card" style={{
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ fontWeight: 800, fontSize: 15 }}>{req.clientName}</div>
                ...
                {processedClientRequests.map(req => (
                  <div key={req.id} className="card" style={{ padding: 14, opacity: 0.7 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ fontWeight: 700, fontSize: 14 }}>{req.clientName}</div>
