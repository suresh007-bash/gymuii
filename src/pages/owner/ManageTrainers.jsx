...
export default function ManageTrainers() {
  ...
  const inp = {width:'100%',padding:'10px 14px',background:'var(--bg-input)',border:'1px solid var(--border)',borderRadius:12,color:'var(--text-primary)',fontSize: 'var(--fs-xs)'};
    ...
    <DashboardLayout title="Manage Trainers">
      ...
      {confirm && (
        <div className="modal-overlay" style={{ zIndex: 1100 }} onClick={() => setConfirm(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 420, textAlign: 'center' }}>
            <div style={{ fontSize: 'var(--fs-hero)', marginBottom: 12 }}><AlertTriangle size={48} color="#f59e0b" /></div>
      ...
      {tab === 'trainers' && (<>
        <div style={{display:'flex',justifyContent:'space-between',marginBottom:20, alignItems: 'center'}}>
          <h2 style={{fontWeight:800, margin: 0}}><Dumbbell size={16} style={{marginRight:4}} /> Trainers ({trainers.length})</h2>
                    ...
                    return (
                      <div key={client.id} style={{
                        ...
                        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                          {isAssignedToThis ? (<>
                            <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--accent-green)', fontWeight: 700 }}>Assigned </span>
                            <button className="btn btn-outline btn-sm" style={{ color: '#ef4444', fontSize: 'var(--fs-xs)', padding: '4px 8px' }} onClick={() => setConfirm({
                              title: ' Remove Client',
          ...
          {trainers.map(t=>{const tc=getTrainerClients(t.id); return (
            <div key={t.id} className="card" style={{animation:'fadeInUp 0.4s ease'}}>
              <div style={{display:'flex',gap:12,alignItems:'center',marginBottom:12}}>
                <div style={{width:48,height:48,borderRadius:'50%',background:'linear-gradient(135deg,#4f46e5,#7c3aed)',display:'flex',alignItems:'center',justifyContent:'center',fontSize: 'var(--fs-sm)',fontWeight:800,color:'#fff'}}>{t.avatar}</div>
                <div><div style={{fontWeight:700,fontSize: 'var(--fs-xs)'}} className="text-truncate">{t.name}</div><div style={{fontSize: 'var(--fs-xs)',color:'var(--text-muted)'}}>{t.specialization||'General'}</div></div>
              </div>
        ...
        {trainers.length === 0 && (
          <div className="card" style={{ textAlign: 'center', padding: 40 }}>
            <div style={{ fontSize: 'var(--fs-hero)', marginBottom: 12 }}><Dumbbell size={48} color="#8b5cf6" /></div>
            <h3 style={{ fontWeight: 800, marginBottom: 8 }}>No Trainers Yet</h3>
            <p style={{ color: 'var(--text-muted)', fontSize: 'var(--fs-xs)' }}>Add trainers or wait for join requests from trainers.</p>
          </div>
        )}
      </>)}
      ...
      {tab === 'requests' && (
        <div style={{ maxWidth: 700, margin: '0 auto' }}>
          ...
          <div style={{ marginBottom: 28 }}>
            <h3 style={{ fontWeight: 800, fontSize: 'var(--fs-sm)', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
              <Dumbbell size={16} style={{marginRight:4}} /> Trainer Join Requests
              {pendingGymRequests.length > 0 && (
                <span style={{
                  padding: '2px 10px', borderRadius: 20, fontSize: 'var(--fs-xs)', fontWeight: 700,
            ...
            {pendingGymRequests.length === 0 && processedGymRequests.length === 0 ? (
              <div className="card" style={{ textAlign: 'center', padding: 32 }}>
                <div style={{ fontSize: 'var(--fs-heading)', marginBottom: 8 }}><Inbox size={36} color="var(--text-muted)" /></div>
                <p style={{ color: 'var(--text-muted)', fontSize: 'var(--fs-xs)' }}>No trainer join requests yet. Trainers can find your gym and send requests to join.</p>
              </div>
            ) : (
                ...
                {pendingGymRequests.map(req => (
                  <div key={req.id} className="card" style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ fontWeight: 800, fontSize: 'var(--fs-xs)' }}>{req.trainerName}</div>
                        <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>{req.trainerSpecialization || 'General Trainer'}</div>
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        <button
                          ...
                          style={{
                            ...
                            fontWeight: 700, fontSize: 'var(--fs-xs)', transition: 'all 0.2s',
                ...
                {processedGymRequests.map(req => (
                  <div key={req.id} className="card" style={{ padding: 14, opacity: 0.7 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ fontWeight: 700, fontSize: 'var(--fs-xs)' }}>{req.trainerName}</div>
                        <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>{req.trainerSpecialization || 'Trainer'}</div>
                      </div>
                      <span className={`badge ${req.status === 'accepted' ? 'badge-green' : 'badge-red'}`} style={{ fontSize: 'var(--fs-xs)' }}>
          ...
          <div>
            ...
            {pendingClientRequests.length === 0 && processedClientRequests.length === 0 ? (
              <div className="card" style={{ textAlign: 'center', padding: 32 }}>
                <div style={{ fontSize: 'var(--fs-heading)', marginBottom: 8 }}><Inbox size={36} color="var(--text-muted)" /></div>
                <p style={{ color: 'var(--text-muted)', fontSize: 'var(--fs-xs)' }}>No client trainer requests yet. Clients can request trainers from the "Join Gym" page.</p>
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  return (
                    <div key={req.id} className="card" style={{
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ fontWeight: 800, fontSize: 'var(--fs-xs)' }}>{req.clientName}</div>
                          <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>
                ...
                {processedClientRequests.map(req => (
                  <div key={req.id} className="card" style={{ padding: 14, opacity: 0.7 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ fontWeight: 700, fontSize: 'var(--fs-xs)' }}>{req.clientName}</div>
                        <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>Requested: {req.trainerName}</div>
                      </div>
                      <span className={`badge ${req.status === 'accepted' ? 'badge-green' : 'badge-red'}`} style={{ fontSize: 'var(--fs-xs)' }}>
                        {req.status === 'accepted' ? ' Accepted' : ' Rejected'}
                      </span>
