      ...
      {confirm && (
        <div className="modal-overlay" style={{ zIndex: 1100 }} onClick={() => setConfirm(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 420, textAlign: 'center' }}>
            <div style={{ fontSize: 'clamp(39px, 1.0vw, 45px)', marginBottom: 12 }}><AlertTriangle size={48} color="#f59e0b" /></div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
        <button
          ...
          style={{
            ...
            fontWeight: 700, fontSize: 'clamp(13px, 1.0vw, 15px)', fontFamily: 'Outfit', transition: 'all 0.3s',
        ...
        >
          ...
          {totalPending > 0 && (
            <span style={{
              ...
              background: '#ef4444', color: '#fff', fontSize: 'clamp(12px, 1.0vw, 14px)', fontWeight: 800,
      ...
      {tab === 'trainers' && (<>
                  ...
                  allUsers.filter(u => u.role === 'client' && u.gymId === user.gymId && !u.blocked).map(client => {
                    ...
                    return (
                      <div key={client.id} style={{
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ fontWeight: 700, fontSize: 'clamp(12px, 1.0vw, 14px)' }} className="text-truncate">{client.name}</div>
                        ...
                        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                          {isAssignedToThis ? (<>
                            <span style={{ fontSize: 'clamp(12px, 1.0vw, 14px)', color: 'var(--accent-green)', fontWeight: 700 }}>Assigned </span>
                            <button className="btn btn-outline btn-sm" style={{ color: '#ef4444', fontSize: 'clamp(12px, 1.0vw, 14px)', padding: '4px 8px' }} onClick={() => setConfirm({
                              title: ' Remove Client',
          ...
          {trainers.map(t=>{const tc=getTrainerClients(t.id); return (
            <div key={t.id} className="card" style={{animation:'fadeInUp 0.4s ease'}}>
              <div style={{display:'flex',gap:12,alignItems:'center',marginBottom:12}}>
                <div style={{width:48,height:48,borderRadius:'50%',background:'linear-gradient(135deg,#4f46e5,#7c3aed)',display:'flex',alignItems:'center',justifyContent:'center',fontSize: 'clamp(15px, 1.0vw, 18px)',fontWeight:800,color:'#fff'}}>{t.avatar}</div>
                <div><div style={{fontWeight:700,fontSize: 'clamp(14px, 1.0vw, 17px)'}} className="text-truncate">{t.name}</div><div style={{fontSize: 'clamp(12px, 1.0vw, 14px)',color:'var(--text-muted)'}}>{t.specialization||'General'}</div></div>
              </div>
        ...
        {trainers.length === 0 && (
          <div className="card" style={{ textAlign: 'center', padding: 40 }}>
            <div style={{ fontSize: 'clamp(39px, 1.0vw, 45px)', marginBottom: 12 }}><Dumbbell size={48} color="#8b5cf6" /></div>
            <h3 style={{ fontWeight: 800, marginBottom: 8 }}>No Trainers Yet</h3>
            <p style={{ color: 'var(--text-muted)', fontSize: 'clamp(12px, 1.0vw, 14px)' }}>Add trainers or wait for join requests from trainers.</p>
          </div>
        )}
      </>)}
      ...
      {tab === 'requests' && (
        <div style={{ maxWidth: 700, margin: '0 auto' }}>
          ...
          <div style={{ marginBottom: 28 }}>
            <h3 style={{ fontFamily: 'Outfit', fontWeight: 800, fontSize: 'clamp(16px, 1.0vw, 19px)', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
              <Dumbbell size={16} style={{marginRight:4}} /> Trainer Join Requests
              {pendingGymRequests.length > 0 && (
                <span style={{
                  padding: '2px 10px', borderRadius: 20, fontSize: 'clamp(12px, 1.0vw, 14px)', fontWeight: 700,
            ...
            {pendingGymRequests.length === 0 && processedGymRequests.length === 0 ? (
              <div className="card" style={{ textAlign: 'center', padding: 32 }}>
                <div style={{ fontSize: 'clamp(30px, 1.0vw, 35px)', marginBottom: 8 }}><Inbox size={36} color="var(--text-muted)" /></div>
                <p style={{ color: 'var(--text-muted)', fontSize: 'clamp(12px, 1.0vw, 14px)' }}>No trainer join requests yet. Trainers can find your gym and send requests to join.</p>
              </div>
            ) : (
                ...
                {pendingGymRequests.map(req => (
                  <div key={req.id} className="card" style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ fontWeight: 800, fontSize: 'clamp(14px, 1.0vw, 17px)' }}>{req.trainerName}</div>
                        <div style={{ fontSize: 'clamp(12px, 1.0vw, 14px)', color: 'var(--text-muted)' }}>{req.trainerSpecialization || 'General Trainer'}</div>
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        <button
                          ...
                          style={{
                            ...
                            fontWeight: 700, fontSize: 'clamp(12px, 1.0vw, 14px)', transition: 'all 0.2s',
                  ...
                  <div key={req.id} className="card" style={{ padding: 14, opacity: 0.7 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <div style={{
                        ...
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        fontSize: 'clamp(13px, 1.0vw, 15px)', fontWeight: 800, color: '#fff',
                      }}>{req.trainerAvatar}</div>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontWeight: 700, fontSize: 'clamp(13px, 1.0vw, 15px)' }}>{req.trainerName}</div>
                        <div style={{ fontSize: 'clamp(12px, 1.0vw, 14px)', color: 'var(--text-muted)' }}>{req.trainerSpecialization || 'Trainer'}</div>
                      </div>
                      <span className={`badge ${req.status === 'accepted' ? 'badge-green' : 'badge-red'}`} style={{ fontSize: 'clamp(12px, 1.0vw, 14px)' }}>
          ...
          <div>
            ...
            {pendingClientRequests.length === 0 && processedClientRequests.length === 0 ? (
              <div className="card" style={{ textAlign: 'center', padding: 32 }}>
                <div style={{ fontSize: 'clamp(30px, 1.0vw, 35px)', marginBottom: 8 }}><Inbox size={36} color="var(--text-muted)" /></div>
                <p style={{ color: 'var(--text-muted)', fontSize: 'clamp(12px, 1.0vw, 14px)' }}>No client trainer requests yet. Clients can request trainers from the "Join Gym" page.</p>
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  return (
                    <div key={req.id} className="card" style={{
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ fontWeight: 800, fontSize: 'clamp(14px, 1.0vw, 17px)' }}>{req.clientName}</div>
                          <div style={{ fontSize: 'clamp(12px, 1.0vw, 14px)', color: 'var(--text-muted)' }}>
                ...
                {processedClientRequests.map(req => (
                  <div key={req.id} className="card" style={{ padding: 14, opacity: 0.7 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ fontWeight: 700, fontSize: 'clamp(13px, 1.0vw, 15px)' }}>{req.clientName}</div>
                        <div style={{ fontSize: 'clamp(12px, 1.0vw, 14px)', color: 'var(--text-muted)' }}>Requested: {req.trainerName}</div>
                      </div>
                      <span className={`badge ${req.status === 'accepted' ? 'badge-green' : 'badge-red'}`} style={{ fontSize: 'clamp(12px, 1.0vw, 14px)' }}>
                        {req.status === 'accepted' ? ' Accepted' : ' Rejected'}
                      </span>
