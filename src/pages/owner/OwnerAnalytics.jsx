        ...
        <div className="card">
          ...
          {['Weight Loss', 'Muscle Gain', 'Maintenance'].map(goal => {
            ...
            return (<div key={goal} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
              <span style={{ fontSize: 'var(--fs-xs)', minWidth: 100 }}>{goal}</span>
              ...
              <span style={{ fontWeight: 800, fontSize: 'var(--fs-xs)' }}>{count}</span>
            </div>);
          })}
        </div>
        <div className="card">
          ...
          trainers.map(t => {
            ...
            return (<div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 0', borderBottom: '1px solid var(--border)' }}>
              <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'var(--gradient-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 'var(--fs-xs)', fontWeight: 800, color: '#fff' }}>{t.avatar}</div>
              <div style={{ flex: 1 }}><div style={{ fontWeight: 700, fontSize: 'var(--fs-xs)' }} className="text-truncate">{t.name}</div><div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>{t.specialization}</div></div>
