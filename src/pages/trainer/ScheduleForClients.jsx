  ...
  if (assignSuccess) return (
    <DashboardLayout title="Schedule Foods to Clients">
      <div style={{ textAlign: 'center', padding: 60 }}>
        ...
        <h2 style={{ fontSize: 'var(--fs-subheading)', fontWeight: 800, marginBottom: 8 }}>Schedule Assigned!</h2>
        <p style={{ color: 'var(--text-muted)', marginBottom: 16, fontSize: 'var(--fs-xs)' }}> {assignSuccess.dates.map(d => fmtDate(d)).join(' • ')}</p>
        <div style={{ display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 24 }}>
          {assignSuccess.clients.map(name => (
            <span key={name} className="badge badge-green text-truncate" style={{ fontSize: 'var(--fs-xs)', padding: '6px 14px' }}> {name}</span>
          ))}
        </div>
        <p style={{ color: 'var(--text-muted)', marginBottom: 24, fontSize: 'var(--fs-xs)' }}>Clients can now view, edit, and order from this schedule.</p>
      ...
      {[{ num: 1, label: 'Select Clients', icon: '' }, { num: 2, label: 'Select Dates', icon: '' }, { num: 3, label: 'Time Slots & Food', icon: '' }].map((s, i) => (
        <div key={s.num} style={{ display: 'flex', alignItems: 'center' }}>
          <div onClick={() => {
            ...
            <span style={{ fontSize: 'var(--fs-body)' }}>{step > s.num ? '' : s.icon}</span><span>{s.label}</span>
          </div>
    ...
    <DashboardLayout title="Schedule Foods to Clients">
      ...
      {step === 1 && (
        <div className="card" style={{ maxWidth: 640, margin: '0 auto' }}>
          ...
          <p style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', marginBottom: 14 }}>Choose the clients you want to assign the meal schedule and targets to.</p>
          
          <div style={{ position: 'relative', marginBottom: 14 }}>
            <input className="form-input" placeholder=" Search clients by name or email..." value={clientSearch} onChange={e => setClientSearch(e.target.value)} style={{ paddingLeft: 36, fontSize: 'var(--fs-sm)', padding: '14px 14px 14px 36px' }} />
          </div>

          {selectedClients.length > 0 && (
            <div style={{ padding: 12, background: 'rgba(34,197,94,0.06)', borderRadius: 10, marginBottom: 14, fontSize: 'var(--fs-xs)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, maxHeight: 400, overflowY: 'auto', marginBottom: 16 }}>
            {clients.length === 0 ? (
              <div style={{ textAlign: 'center', padding: 30, color: 'var(--text-muted)', fontSize: 'var(--fs-xs)' }}>
              ...
              clients.filter(c => c.name.toLowerCase().includes(clientSearch.toLowerCase()) || c.email?.toLowerCase().includes(clientSearch.toLowerCase())).map(client => {
                ...
                return (
                  <div key={client.id} onClick={() => toggleClient(client.id)} style={{
                    ...
                    <div style={{ width: 24, height: 24, borderRadius: 6, border: `2px solid ${isSelected ? 'var(--accent-orange)' : 'var(--border)'}`, background: isSelected ? 'var(--accent-orange)' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 'var(--fs-xs)', fontWeight: 900, flexShrink: 0 }}>
                      {isSelected && '✓'}
                    </div>
                    <div style={{ width: 44, height: 44, borderRadius: '50%', background: 'linear-gradient(135deg,#4f46e5,#7c3aed)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 'var(--fs-sm)', fontWeight: 800, color: '#fff', flexShrink: 0 }}>{client.avatar}</div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: 800, fontSize: 'var(--fs-sm)', marginBottom: 4 }} className="text-truncate">{client.name}</div>
                      <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>{client.email} • Target: <span style={{ color: 'var(--accent-orange)', fontWeight: 700 }}>{client.goal || 'Not Set'}</span></div>
                    </div>
                  </div>
                );
              })
          ...
          <button className="btn btn-primary btn-lg" style={{ width: '100%', fontSize: 'var(--fs-sm)', padding: '14px' }}
            onClick={() => setStep(2)}
      ...
      {step === 2 && (
        <div className="card" style={{ maxWidth: 640, margin: '0 auto' }}>
          <div className="card-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button className="btn btn-outline btn-sm" onClick={() => setStep(1)} style={{ fontSize: 'var(--fs-xs)' }}>← Back</button>
          </div>
          <p style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', marginBottom: 14 }}>Pick dates on the calendar. You can schedule multiple slots and foods for each date.</p>
          
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, minmax(40px, 1fr))', gap: 6, marginBottom: 16 }}>
            {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(d => (
              <div key={d} style={{ textAlign: 'center', fontSize: 'var(--fs-xs)', fontWeight: 800, color: 'var(--text-muted)', padding: 6 }}>{d}</div>
            ))}
            ...
            {calendarDays.map(dateStr => {
              ...
              return (
                <button key={dateStr} onClick={() => toggleDate(dateStr)} style={{
                  ...
                  cursor: 'pointer', fontSize: 'var(--fs-sm)', fontWeight: 800, transition: 'all 0.2s',
          ...
          {selectedDates.length > 0 && (
            <div style={{ padding: 14, background: 'rgba(34,197,94,0.06)', borderRadius: 10, marginBottom: 14, fontSize: 'var(--fs-xs)' }}>
      ...
      {step === 3 && (
        <div>
              ...
              <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 800, width: '95vw', display: 'flex', flexDirection: 'column', maxHeight: '90vh' }}>
                ...
                <div style={{ display: 'flex', gap: 8, marginBottom: 10, alignItems: 'center' }}>
                  <div style={{ flex: 1, position: 'relative' }}>
                    ...
                    <input className="form-input" placeholder="Search food..." value={search} onChange={e => setSearch(e.target.value)} style={{ paddingLeft: 32, fontSize: 'var(--fs-sm)', padding: '14px 14px 14px 32px' }} />
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', gap: 8, marginBottom: 10, overflowX: 'auto', paddingBottom: 4 }}>
                  {CATEGORIES.map(c => (
                    <button key={c} onClick={() => setFoodCat(c)} style={{
                      padding: '8px 16px', borderRadius: 20, border: 'none', cursor: 'pointer', fontSize: 'var(--fs-xs)', fontWeight: 700, whiteSpace: 'nowrap',
                ...
                <div style={{ display: 'flex', gap: 6, marginBottom: 14 }}>
                  {[
                    ...
                    <button key={f.key} onClick={() => setNutriSort(nutriSort === f.key ? null : f.key)} style={{
                      padding: '8px 14px', borderRadius: 8, fontSize: 'var(--fs-xs)', fontWeight: 700, cursor: 'pointer',
                  ...
                  {filteredMenu.map(item => {
                    ...
                    return (
                      <div key={item.id} style={{
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ fontWeight: 800, fontSize: 'var(--fs-sm)' }} className="text-truncate">{item.name}</div>
                          <div style={{ display: 'flex', gap: 12, fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', marginTop: 4 }}>
                          ...
                          <div style={{ fontSize: 'var(--fs-sm)', fontWeight: 800, color: 'var(--accent-green)', marginTop: 4 }}>₹{item.price}</div>
                        </div>
                        {inSlot ? (
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            <button className="btn btn-outline" style={{ width: 36, height: 36, padding: 0, fontSize: 'var(--fs-sm)', display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => { if (inSlot.qty <= 1) removeItem(activeDate, activeSlotId, item.id); else updateQty(activeDate, activeSlotId, item.id, -1); }}>−</button>
                            <span style={{ fontWeight: 900, fontSize: 'var(--fs-body)', minWidth: 26, textAlign: 'center' }}>{inSlot.qty}</span>
                            <button className="btn btn-outline" style={{ width: 36, height: 36, padding: 0, fontSize: 'var(--fs-sm)', display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => updateQty(activeDate, activeSlotId, item.id, 1)}>+</button>
                          </div>
                        ) : (
                          <button className="btn btn-primary" style={{ padding: '10px 22px', fontSize: 'var(--fs-xs)', fontWeight: 800 }} onClick={() => addItem(activeDate, activeSlotId, item)}>+ Add</button>
                        )}
                      </div>
                    );
                  })}
                ...
                <div className="modal-footer" style={{ position: 'sticky', bottom: 0, marginTop: 0, padding: '12px 18px', background: 'var(--bg-primary)', borderTop: '1.5px solid var(--border)', zIndex: 5, flexShrink: 0 }}>
                  ...
                  <button className="btn btn-primary" style={{ fontSize: 'var(--fs-sm)', padding: '12px 32px' }} onClick={() => { setShowFoodPicker(false); setSlotSnapshot(null); setSearch(''); setFoodCat('All'); setNutriSort(null); }}> Done</button>
                </div>
              </div>
            </div>
          )}
          ...
          <div style={{ marginTop: 24, textAlign: 'center' }}>
            <button className="btn btn-success btn-lg" style={{ minWidth: 300, fontSize: 'var(--fs-sm)' }}
              onClick={handleAssign}
              disabled={filledDates.length === 0}>
               Publish Food Schedule
            </button>
            <p style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', marginTop: 8 }}>This will instantly update the food schedules for the selected clients.</p>
          </div>
        </div>
      )}
    </DashboardLayout>
