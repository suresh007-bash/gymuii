...
export default function MyOwner() {
  ...
  const labelStyle = { fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', fontWeight: 600, marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.5px' };
  const valueStyle = { fontSize: 'var(--fs-xs)', fontWeight: 700, color: 'var(--text-primary)' };
      ...
      return (
        <DashboardLayout title="My Gym & Owner">
          ...
          <button onClick={() => { setView('browse'); setSelectedGym(null); setImageIndex(0); }} style={{
            ...
            fontSize: 'var(--fs-xs)', fontWeight: 700, display: 'flex', alignItems: 'center', gap: 6, marginBottom: 16, padding: 0,
          }}>
            ← Back to Gyms
          </button>
          ...
          {selectedGym.images && selectedGym.images.length > 0 && (
            <div style={{ position: 'relative', borderRadius: 20, overflow: 'hidden', marginBottom: 20, height: 'clamp(180px, 10.3vw, 207px)' }}>
              ...
              {selectedGym.images.length > 1 && (
                <>
                  <button onClick={() => setImageIndex(i => (i - 1 + selectedGym.images.length) % selectedGym.images.length)} style={{
                    ...
                    background: 'rgba(0,0,0,0.5)', color: '#fff', fontSize: 'var(--fs-sm)', display: 'flex', alignItems: 'center', justifyContent: 'center',
                  }}>‹</button>
              ...
              <div style={{ position: 'absolute', bottom: 20, left: 20 }}>
                <h2 style={{ color: '#fff', fontWeight: 900, fontSize: 'var(--fs-subheading)', margin: 0 }}>{selectedGym.name}</h2>
                <div style={{ color: 'rgba(255,255,255,0.8)', fontSize: 'var(--fs-xs)', marginTop: 4 }}><Icon icon={MapPin} size={13} style={{marginRight:4}} /> {selectedGym.location}</div>
              </div>
            </div>
          )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(100px, 1fr))', gap: 12, marginBottom: 20 }}>
            <div className="card" style={{ textAlign: 'center', padding: 14 }}>
              <div style={{ fontSize: 'var(--fs-body)', fontWeight: 900, color: '#f97316' }}><Icon icon={Star} size={20} color="#f97316" style={{marginRight:4}} /> {selectedGym.rating}</div>
              <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', fontWeight: 600 }}>Rating</div>
            </div>
            <div className="card" style={{ textAlign: 'center', padding: 14 }}>
              <div style={{ fontSize: 'var(--fs-body)', fontWeight: 900, color: '#3b82f6' }}><Icon icon={Users} size={20} color="#3b82f6" style={{marginRight:4}} /> {gymClientsForGym.length || selectedGym.members}</div>
              <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', fontWeight: 600 }}>Members</div>
            </div>
            <div className="card" style={{ textAlign: 'center', padding: 14 }}>
              <div style={{ fontSize: 'var(--fs-body)', fontWeight: 900, color: '#22c55e' }}><Icon icon={Dumbbell} size={20} color="#22c55e" style={{marginRight:4}} /> {gymTrainersForGym.length || selectedGym.trainers}</div>
              <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', fontWeight: 600 }}>Trainers</div>
            </div>
            <div className="card" style={{ textAlign: 'center', padding: 14 }}>
              <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 700, color: 'var(--text-primary)' }}><Icon icon={Clock} size={14} /></div>
              <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', fontWeight: 600, marginTop: 4 }}>{selectedGym.hours || 'N/A'}</div>
            </div>
          </div>
          ...
          {selectedGym.description && (
            <div className="card" style={{ marginBottom: 20 }}>
              <h3 style={{ fontWeight: 800, fontSize: 'var(--fs-sm)', marginBottom: 10 }}><Icon icon={FileText} size={16} style={{marginRight:6}} /> About</h3>
              <p style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-secondary)', lineHeight: 1.7 }}>{selectedGym.description}</p>
            </div>
          )}
          ...
          {selectedGym.amenities && (
            <div className="card" style={{ marginBottom: 20 }}>
              <h3 style={{ fontWeight: 800, fontSize: 'var(--fs-sm)', marginBottom: 12 }}><Icon icon={Target} size={16} style={{marginRight:6}} /> Amenities</h3>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                {selectedGym.amenities.map((a, i) => (
                  <span key={i} style={{
                    padding: '6px 14px', borderRadius: 20, fontSize: 'var(--fs-xs)', fontWeight: 600,
          ...
          {gymTrainersForGym.length > 0 && (
            <div className="card" style={{ marginBottom: 20 }}>
              <h3 style={{ fontWeight: 800, fontSize: 'var(--fs-sm)', marginBottom: 12 }}><Icon icon={Dumbbell} size={16} style={{marginRight:6}} /> Current Trainers</h3>
                ...
                {gymTrainersForGym.map(t => (
                  <div key={t.id} style={{
                    ...
                    <div style={{
                      ...
                      fontSize: 'var(--fs-xs)', fontWeight: 800, color: '#fff', flexShrink: 0,
                    }}>{t.avatar}</div>
                    <div>
                      <div style={{ fontWeight: 700, fontSize: 'var(--fs-xs)' }} className="text-truncate">{t.name}</div>
                      <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>{t.specialization || 'Trainer'}</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div style={{ marginBottom: 20 }}>
            {reqStatus === 'pending' ? (
              <div style={{
                ...
                color: '#f59e0b', fontWeight: 700, fontSize: 'var(--fs-xs)',
              ...
              <div style={{
                ...
                color: '#22c55e', fontWeight: 700, fontSize: 'var(--fs-xs)',
              }}>
              ...
              <button
                ...
                style={{
                  ...
                  color: '#fff', fontWeight: 800, fontSize: 'var(--fs-xs)', cursor: 'pointer',
      ...
      <DashboardLayout title="My Gym & Owner">
        ...
        <div className="card" style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <div style={{ fontSize: 'var(--fs-heading)' }}><Icon icon={Building2} size={36} color="#22c55e" /></div>
            <div>
              <h3 style={{ fontWeight: 800, fontSize: 'var(--fs-sm)', marginBottom: 4 }}>Browse Available Gyms</h3>
        ...
        {joinRequests.filter(r => r.trainerId === user?.id && r.status === 'pending').length > 0 && (
          <div style={{
            ...
            fontSize: 'var(--fs-xs)', color: '#f59e0b', fontWeight: 600,
              ...
              >
                ...
                {gymItem.images && gymItem.images[0] && (
                  <div style={{ height: 160, overflow: 'hidden', position: 'relative' }}>
                    ...
                    {reqStatus === 'pending' && (
                      <div style={{ position: 'absolute', top: 10, left: 10, padding: '4px 10px', borderRadius: 8, background: 'rgba(251,191,36,0.9)', color: '#fff', fontSize: 'var(--fs-xs)', fontWeight: 700 }}>
                         PENDING
                      </div>
                    )}
                  </div>
                ...
                <div style={{ padding: 16 }}>
                  <h4 style={{ fontWeight: 800, fontSize: 'var(--fs-sm)', marginBottom: 6 }} className="text-truncate">{gymItem.name}</h4>
                  <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 4 }}>
                    <Icon icon={MapPin} size={12} style={{marginRight:4}} /> {gymItem.location}
                  </div>
                  <div style={{ display: 'flex', gap: 12, marginBottom: 12 }}>
                    <div style={{ padding: '6px 12px', borderRadius: 8, background: 'var(--bg-tertiary)', fontSize: 'var(--fs-xs)', fontWeight: 600 }}>
    ...
    <DashboardLayout title="My Gym & Owner">
      ...
      <div style={{ ...cardStyle, marginBottom: 20, background: 'linear-gradient(135deg, rgba(34,197,94,0.06), rgba(249,115,22,0.06))', border: '1px solid rgba(34,197,94,0.15)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 20 }}>
          ...
          <div>
            <h2 style={{ fontWeight: 900, fontSize: 'var(--fs-body)', margin: 0 }}>{gym?.name || 'My Gym'}</h2>
            <p style={{ color: 'var(--text-muted)', fontSize: 'var(--fs-xs)', margin: 0 }}>Your current gym</p>
          </div>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 12 }}>
          <div style={{ background: 'var(--bg-secondary)', borderRadius: 12, padding: 14, textAlign: 'center', border: '1px solid var(--border)' }}>
            <div style={{ fontSize: 'var(--fs-subheading)', fontWeight: 900, color: '#22c55e' }}>{gymTrainers.length}</div>
            <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', fontWeight: 600 }}>Trainers</div>
          </div>
          <div style={{ background: 'var(--bg-secondary)', borderRadius: 12, padding: 14, textAlign: 'center', border: '1px solid var(--border)' }}>
            <div style={{ fontSize: 'var(--fs-subheading)', fontWeight: 900, color: '#3b82f6' }}>{gymClients.length}</div>
            <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', fontWeight: 600 }}>Members</div>
          </div>
          {gym?.gst && (
            <div style={{ background: 'var(--bg-secondary)', borderRadius: 12, padding: 14, textAlign: 'center', border: '1px solid var(--border)' }}>
              <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 700, color: 'var(--text-primary)' }}>{gym.gst}</div>
              <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', fontWeight: 600 }}>GST No.</div>
            </div>
          )}
        </div>
      </div>
      ...
      {owner && (
        <div style={{ ...cardStyle, marginBottom: 20 }}>
          <h3 style={{ fontWeight: 800, fontSize: 'var(--fs-sm)', marginBottom: 16 }}><Icon icon={Crown} size={16} style={{marginRight:6}} /> Gym Owner</h3>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 20 }}>
            <div style={{
              width: 64, height: 64, borderRadius: '50%',
              background: 'linear-gradient(135deg, #eab308, #f59e0b)',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: 'var(--fs-body)', fontWeight: 900, color: '#fff',
      ...
      <div style={{ ...cardStyle, border: '1px solid rgba(239,68,68,0.2)', background: 'rgba(239,68,68,0.02)' }}>
        <h3 style={{ fontWeight: 800, fontSize: 'var(--fs-sm)', marginBottom: 8, color: '#ef4444' }}><Icon icon={LogOut} size={16} style={{marginRight:6}} /> Leave Gym</h3>
        <p style={{ color: 'var(--text-muted)', fontSize: 'var(--fs-xs)', marginBottom: 16, lineHeight: 1.6 }}>
