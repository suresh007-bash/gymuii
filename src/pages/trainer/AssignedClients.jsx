        ...
        <div className="modal-overlay" onClick={() => setViewProfileClient(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 560 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 20, paddingBottom: 16, borderBottom: '1px solid var(--border)' }}>
              ...
              <div>
                <h4 style={{ margin: 0, fontSize: 'calc(22px + 0.5vw)', fontWeight: 800 }}>{profileForm.name}</h4>
                <p style={{ margin: 0, fontSize: 'calc(17px + 0.5vw)', color: 'var(--text-muted)' }}>Joined: {profileForm.joinDate || 'N/A'}</p>
              </div>
            </div>
          ...
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 440 }}>
            ...
            <div style={{ maxHeight: 300, overflowY: 'auto', marginBottom: 12, padding: 8 }}>
              {(messages[msgClient.id] || []).length === 0 ? (
                <div style={{ textAlign: 'center', padding: 30, color: 'var(--text-muted)', fontSize: 'calc(17px + 0.5vw)' }}>No messages yet. Start a conversation!</div>
              ) : (
                (messages[msgClient.id] || []).map((msg, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: msg.sender === 'trainer' ? 'flex-end' : 'flex-start', marginBottom: 8 }}>
                    <div style={{
                      maxWidth: '70%', padding: '8px 12px', borderRadius: 12, fontSize: 'calc(17px + 0.5vw)',
                      background: msg.sender === 'trainer' ? 'var(--accent-orange)' : 'var(--bg-tertiary)',
                      color: msg.sender === 'trainer' ? '#fff' : 'var(--text-primary)',
                    }}>
                      {msg.text}
                      <div style={{ fontSize: 'calc(13px + 0.5vw)', opacity: 0.7, marginTop: 2 }}>{msg.time}</div>
                    </div>
                  </div>
                ))
              )}
          ...
          <div style={{ display: 'grid', gap: 10 }}>
            {pendingRequests.map(req => (
              <div key={req.id} style={{
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 800, fontSize: 'calc(18px + 0.5vw)', marginBottom: 2 }}>{req.clientName}</div>
                  <div style={{ fontSize: 'calc(16px + 0.5vw)', color: 'var(--text-muted)', display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button
                    ...
                    style={{
                      ...
                      color: '#fff', fontWeight: 700, fontSize: 'calc(16px + 0.5vw)', padding: '8px 16px',
                  ...
                  <button
                    ...
                    style={{
                      ...
                      color: '#ef4444', fontWeight: 700, fontSize: 'calc(16px + 0.5vw)', padding: '8px 16px',
                      borderRadius: 10, cursor: 'pointer', fontFamily: 'Outfit',
                    }}
            ...
            <tr key={c.id}>
              <td>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }} onClick={() => openClientProfile(c)}>
                  <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'linear-gradient(135deg,#4f46e5,#7c3aed)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 'calc(16px + 0.5vw)', fontWeight: 800, color: '#fff' }}>{c.avatar}</div>
                  <div>
                    <div style={{ fontWeight: 700, fontSize: 'calc(17px + 0.5vw)', textDecoration: 'underline' }}>{c.name}</div>
                    <div style={{ fontSize: 'calc(15px + 0.5vw)', color: 'var(--text-muted)' }}>{c.email}</div>
                  </div>
                </div>
              </td>
