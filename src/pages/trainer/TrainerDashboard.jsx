  ...
  return (
    <DashboardLayout title="Dashboard">
      ...
      <div style={{ borderRadius: 20, overflow: 'hidden', position: 'relative', marginBottom: 24, background: `linear-gradient(rgba(0,0,0,0.45), rgba(0,0,0,0.55)), url(${trainerWorkout}) center/cover`, padding: '48px 40px', color: '#fff', width: '100%', boxSizing: 'border-box' }}>
        <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 800, background: '#22c55e', display: 'inline-block', padding: '4px 14px', borderRadius: 20, marginBottom: 12, letterSpacing: 1 }}><Icon icon={Dumbbell} size={12} style={{marginRight:4}} /> TRAINER PORTAL</div>
        <h1 style={{ fontSize: 'var(--fs-heading)', fontWeight: 900, lineHeight: 1.15, marginBottom: 12, maxWidth: 450 }}>Welcome, {user?.name?.split(' ')[0]}!</h1>
        <p style={{ fontSize: 'var(--fs-xs)', opacity: 0.85, maxWidth: 420, lineHeight: 1.6 }}>{user?.specialization} • {clients.length} active clients • {plans.length} diet plans created</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 280px), 1fr))', gap: 20, marginBottom: 24, width: '100%' }}>
        <div className="card" style={{ borderLeft: '4px solid #f97316', margin: 0 }}>
          <h4 style={{ fontWeight: 800, fontSize: 'var(--fs-xs)', marginBottom: 6 }}><Icon icon={Calendar} size={14} style={{marginRight:4}} /> Client Meal Scheduling</h4>
          <p style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', lineHeight: 1.6, marginBottom: 10 }}>Create and manage meal schedules for your clients. Set breakfast, lunch & dinner for any dates.</p>
          <Link to="/trainer/meal-scheduling" style={{ fontSize: 'var(--fs-xs)', color: 'var(--accent-orange)', fontWeight: 700, textDecoration: 'none' }}>Open Scheduler →</Link>
        </div>
        <div className="card" style={{ borderLeft: '4px solid #22c55e', margin: 0 }}>
          <h4 style={{ fontWeight: 800, fontSize: 'var(--fs-xs)', marginBottom: 6 }}><Icon icon={Salad} size={14} style={{marginRight:4}} /> Diet Plans</h4>
          <p style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', lineHeight: 1.6, marginBottom: 10 }}>Create personalized nutrient packs and send them directly to your clients for easy ordering.</p>
          <Link to="/trainer/diet-plans" style={{ fontSize: 'var(--fs-xs)', color: 'var(--accent-green)', fontWeight: 700, textDecoration: 'none' }}>Create Diet Plan →</Link>
        </div>
      </div>
        ...
        <div className="mind-categories-scroller">
          {MIND_CATS.map((cat, i) => (
            <Link key={i} to={i < 2 ? '/trainer/diet-plans' : '/trainer/meal-scheduling'} style={{ textDecoration: 'none', textAlign: 'center', flexShrink: 0, width: 86 }}>
              ...
              <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 700, color: 'var(--text-primary)', whiteSpace: 'normal', lineHeight: 1.2 }}>{cat.label}</div>
            </Link>
          ))}
        </div>
      </div>
      ...
      <div className="card" style={{ marginBottom: 20, width: '100%', boxSizing: 'border-box' }}>
        <div className="card-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><h3 className="card-title" style={{ display: 'flex', alignItems: 'center' }}><Icon icon={Users} size={16} style={{marginRight:6}} /> My Clients ({clients.length})</h3><Link to="/trainer/clients" style={{ color: 'var(--accent-orange)', fontSize: 'var(--fs-xs)', fontWeight: 700, textDecoration: 'none' }}>View All →</Link></div>
        {clients.length === 0 ? <p style={{ color: 'var(--text-muted)', padding: 12, fontSize: 'var(--fs-xs)' }}>No clients assigned</p> :
        clients.slice(0, 5).map(c => (
          <div key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 0', borderBottom: '1px solid var(--border)' }}>
            <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'var(--gradient-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 'var(--fs-xs)', fontWeight: 800, color: '#fff' }}>{c.avatar}</div>
            <div style={{ flex: 1 }}><div style={{ fontWeight: 700, fontSize: 'var(--fs-xs)' }} className="text-truncate">{c.name}</div><div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>{c.goal} • {c.diet}</div></div>
