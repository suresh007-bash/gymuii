...
const MEALS = [
  ...
  { key: 'noon', label: 'Lunch', icon: '', time: '12:30 PM', color: '#22c55e' },
    ...
    <DashboardLayout title="Weekly Meal Schedule">
      ...
      {editCell && (
        <div className="modal-overlay" onClick={() => { setEditCell(null); setSelectedItems([]); setSearch(''); }}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 480 }}>
            ...
            <div style={{ fontSize: 'clamp(12px, 3vw, 16px)', color: 'var(--text-muted)', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
              ...
              <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <input
                  ...
                  style={{
                    ...
                    background: 'var(--bg-input)', color: 'var(--text-primary)', fontSize: 'clamp(12px, 3vw, 16px)',
              ...
              {filteredMenu.map(item => (
                <div key={item.id} onClick={() => toggleItem(item.id)} style={{
                  ...
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    ...
                    <div>
                      <div style={{ fontWeight: 700, fontSize: 'clamp(12px, 3vw, 16px)' }}>{selectedItems.includes(item.id) ? <><Icon icon={CheckCircle2} size={11} style={{marginRight:3}} /> </> : ''}{item.name}</div>
                      <div style={{ fontSize: 'clamp(12px, 3vw, 14px)', color: 'var(--text-muted)' }}><Icon icon={Flame} size={10} style={{marginRight:1}} />{item.calories} <Icon icon={Beef} size={10} style={{marginRight:1}} />{item.protein}g • ₹{item.price}</div>
                    </div>
                  </div>
                </div>
              ))}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          <label className="form-label" style={{ margin: 0, fontSize: 'clamp(12px, 3vw, 17px)' }}>Client:</label>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontFamily: 'Outfit', fontWeight: 800, fontSize: 'clamp(14px, 3vw, 19px)', minWidth: 200, textAlign: 'center' }}><Icon icon={Calendar} size={14} style={{marginRight:4}} /> {weekLabel}</span>
          ...
          <input type="date" style={{ padding: '5px 10px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--bg-input)', color: 'var(--text-primary)', fontSize: 'clamp(12px, 3vw, 16px)' }}
            onChange={e => {
      ...
      <div className="card" style={{ overflow: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 4 }}>
          <thead>
            <tr>
              <th style={{ width: 80, padding: 10, fontSize: 'clamp(12px, 3vw, 16px)', fontWeight: 800, color: 'var(--text-muted)' }}></th>
              {DAYS.map((day, i) => {
                const date = weekDates[i];
                const isToday = date === new Date().toISOString().split('T')[0];
                return (
                  <th key={day} style={{
                    padding: '10px 6px', textAlign: 'center', fontSize: 'clamp(12px, 3vw, 16px)',
                    background: isToday ? 'rgba(249,115,22,0.08)' : 'transparent',
                    borderRadius: 10,
                  }}>
                    <div style={{ fontWeight: 900, fontSize: 'clamp(12px, 3vw, 17px)', color: isToday ? 'var(--accent-orange)' : 'var(--text-primary)' }}>{day.slice(0, 3)}</div>
                    <div style={{ fontSize: 'clamp(12px, 3vw, 15px)', color: 'var(--text-muted)' }}>{new Date(date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}</div>
                  </th>
                );
              })}
            </tr>
              ...
              <tr key={meal.key}>
                ...
                <td style={{ padding: '8px 6px', verticalAlign: 'top' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ fontSize: 'clamp(16px, 3vw, 22px)' }}>{meal.icon}</span>
                    <div>
                      <div style={{ fontWeight: 800, fontSize: 'clamp(12px, 3vw, 16px)', color: meal.color }}>{meal.label}</div>
                      <div style={{ fontSize: 'clamp(12px, 3vw, 14px)', color: 'var(--text-muted)' }}>{meal.time}</div>
                    </div>
                  </div>
                </td>
                      ...
                      >
                        {sched ? (
                          <>
                            {items.slice(0, 2).map((item, idx) => (
                              <div key={idx} style={{ display: 'flex', gap: 4, alignItems: 'center', marginBottom: 4 }}>
                                ...
                                <span style={{ fontSize: 'clamp(12px, 3vw, 14px)', fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.name}</span>
                              </div>
                            ))}
                            {items.length > 2 && <div style={{ fontSize: 'clamp(12px, 3vw, 13px)', color: 'var(--text-muted)' }}>+{items.length - 2} more</div>}
                            <div style={{ fontSize: 'clamp(12px, 3vw, 13px)', color: meal.color, fontWeight: 800, marginTop: 4 }}><Icon icon={Flame} size={9} style={{marginRight:1}} /> {totalMealCal} kcal</div>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', minHeight: 70 }}>
                            <span style={{ fontSize: 'clamp(18px, 3vw, 24px)', opacity: 0.2 }}>+</span>
                          </div>
                        )}
                      </div>
                    </td>
      ...
      <div style={{ display: 'flex', gap: 20, marginTop: 12, justifyContent: 'center' }}>
        {MEALS.map(m => (
          <div key={m.key} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 'clamp(12px, 3vw, 16px)' }}>
            <div style={{ width: 10, height: 10, borderRadius: 3, background: m.color }} />
            <span style={{ fontWeight: 600 }}>{m.icon} {m.label}</span>
          </div>
        ))}
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 'clamp(12px, 3vw, 16px)' }}>
