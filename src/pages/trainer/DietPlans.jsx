  ...
  return (
    <DashboardLayout title="Diet Plans">
      <div className="card" style={{ marginBottom: 20 }}>
        ...
        {step === 1 && (<div>
          <p style={{ marginBottom: 12, fontSize: 'clamp(12px, 1.0vw, 14px)', color: 'var(--text-muted)' }}>Select members to assign this plan:</p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>{clients.map(c => (
            <label key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: selectedClients.includes(c.id) ? 'rgba(249,115,22,0.08)' : 'var(--bg-tertiary)', borderRadius: 10, border: `1px solid ${selectedClients.includes(c.id) ? 'var(--accent-orange)' : 'var(--border)'}`, cursor: 'pointer', transition: 'all 0.2s' }}>
              ...
              <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'var(--gradient-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 'clamp(12px, 1.0vw, 14px)', fontWeight: 800, color: '#fff' }}>{c.avatar}</div>
              <div><span style={{ fontWeight: 700, fontSize: 'clamp(12px, 1.0vw, 14px)' }} className="text-truncate">{c.name}</span><br/><span style={{ fontSize: 'clamp(12px, 1.0vw, 14px)', color: 'var(--text-muted)' }}>{c.goal} • {c.diet}</span></div>
            </label>
          ))}</div>}
        ...
        {step === 2 && (<div>
          <p style={{ marginBottom: 12, fontSize: 'clamp(12px, 1.0vw, 14px)', color: 'var(--text-muted)' }}>Select food items for the plan:</p>
          <div className="food-grid">{MENU_ITEMS.filter(m => m.available).map(item => (
            <div key={item.id} onClick={() => toggleItem(item.id)} style={{ padding: 14, background: selectedItems.includes(item.id) ? 'rgba(249,115,22,0.08)' : 'var(--bg-tertiary)', borderRadius: 12, border: `1px solid ${selectedItems.includes(item.id) ? 'var(--accent-orange)' : 'var(--border)'}`, cursor: 'pointer', transition: 'all 0.2s' }}>
              <div style={{ fontWeight: 700, fontSize: 'clamp(12px, 1.0vw, 14px)' }}>{selectedItems.includes(item.id) ? <><Icon icon={CheckCircle2} size={12} style={{marginRight:4}} /> </> : ''}{item.name}</div>
              <div style={{ fontSize: 'clamp(12px, 1.0vw, 14px)', color: 'var(--text-muted)' }}><Icon icon={Flame} size={11} style={{marginRight:2}} /> {item.calories} kcal • <Icon icon={Beef} size={11} style={{marginRight:2}} /> {item.protein}g • ₹{item.price}</div>
            </div>
          ))}</div>
        ...
        {step === 3 && (<div>
          ...
          <p style={{ fontSize: 'clamp(12px, 1.0vw, 14px)', color: 'var(--text-muted)' }}>Sending to: <strong>{selectedClients.length}</strong> member(s)</p>
          <p style={{ fontSize: 'clamp(12px, 1.0vw, 14px)', color: 'var(--text-muted)' }}>Items: <strong>{selectedItems.length}</strong> food item(s)</p>
