    ...
    return (
      <div className="card" style={{ marginBottom: 12 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
          <span style={{ fontWeight: 700, fontSize: 'var(--fs-xs)', display: 'flex', alignItems: 'center', gap: 4 }}>{icon} {label}</span>
          <span style={{ fontWeight: 800 }}>{current} / {target}{label === 'Calories' ? ' kcal' : 'g'}</span>
        </div>
        ...
        <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', marginTop: 4 }}>{Math.round(pct)}% of daily target</div>
      </div>
    );
  };
