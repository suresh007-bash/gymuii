      ...
      <div style={{ minHeight: '112vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, background: 'linear-gradient(135deg, #0f172a, #1e293b)' }}>
        <div style={{ maxWidth: 400, width: '100%', textAlign: 'center', animation: 'scaleIn 0.5s cubic-bezier(0.16,1,0.3,1)' }}>
          <div style={{
            ...
            <div style={{ width: 56, height: 56, borderRadius: '50%', background: 'linear-gradient(135deg, #f97316, #22c55e)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px', fontSize: 'clamp(19px, 1.0vw, 22px)', fontWeight: 900, color: '#fff' }}>{user.avatar || '?'}</div>
            <h2 style={{ color: '#fff', fontSize: 'clamp(18px, 1.0vw, 21px)', fontWeight: 800, marginBottom: 4 }}>Welcome back, {user.name?.split(' ')[0]}!</h2>
            <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 'clamp(12px, 1.0vw, 14px)', marginBottom: 24 }}>You're signed in as <span style={{ color: '#f97316', fontWeight: 700 }}>{user.role?.toUpperCase()}</span></p>
            <button onClick={() => navigate(roleMap[user.role])} style={{ width: '100%', padding: 14, background: 'linear-gradient(135deg, #f97316, #fb923c)', color: '#fff', borderRadius: 12, fontSize: 'clamp(14px, 1.0vw, 17px)', fontWeight: 800, cursor: 'pointer', border: 'none', marginBottom: 10, boxShadow: '0 4px 20px rgba(249,115,22,0.4)' }}>
              Continue to Dashboard →
            </button>
            <button onClick={() => { logout(); }} style={{ width: '100%', padding: 12, background: 'rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.6)', borderRadius: 12, fontSize: 'clamp(12px, 1.0vw, 14px)', fontWeight: 700, cursor: 'pointer', border: '1px solid rgba(255,255,255,0.1)' }}>
    ...
    <div style={{ minHeight: '112vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ maxWidth: 400, width: '100%', animation: 'scaleIn 0.5s cubic-bezier(0.16,1,0.3,1)', position: 'relative', zIndex: 10 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          ...
          <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 'clamp(12px, 1.0vw, 14px)' }}>Sign in to your account</p>
        </div>
        ...
        <div style={{
          ...
          <form onSubmit={handleLogin}>
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: 'block', fontSize: 'clamp(12px, 1.0vw, 14px)', fontWeight: 700, color: 'rgba(255,255,255,0.6)', marginBottom: 6, textTransform: 'uppercase', letterSpacing: 0.5 }}>Email</label>
              <input type="email" value={email} onChange={e => { setEmail(e.target.value); setError(''); }} placeholder="Enter your email" required style={{ width: '100%', padding: '12px 16px', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 12, color: '#fff', fontSize: 'clamp(13px, 1.0vw, 15px)', outline: 'none', transition: 'border 0.3s' }} onFocus={e => e.target.style.borderColor = 'rgba(249,115,22,0.6)'} onBlur={e => e.target.style.borderColor = 'rgba(255,255,255,0.12)'} />
            </div>
            <div style={{ marginBottom: 20 }}>
              <label style={{ display: 'block', fontSize: 'clamp(12px, 1.0vw, 14px)', fontWeight: 700, color: 'rgba(255,255,255,0.6)', marginBottom: 6, textTransform: 'uppercase', letterSpacing: 0.5 }}>Password</label>
              <input type="password" value={password} onChange={e => { setPassword(e.target.value); setError(''); }} placeholder="Enter your password" required style={{ width: '100%', padding: '12px 16px', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 12, color: '#fff', fontSize: 'clamp(13px, 1.0vw, 15px)', outline: 'none', transition: 'border 0.3s' }} onFocus={e => e.target.style.borderColor = 'rgba(249,115,22,0.6)'} onBlur={e => e.target.style.borderColor = 'rgba(255,255,255,0.12)'} />
            </div>
          ...
          <p style={{ textAlign: 'center', color: 'rgba(255,255,255,0.4)', fontSize: 'clamp(12px, 1.0vw, 14px)', marginTop: 16 }}>Don't have an account? <Link to="/register" style={{ color: '#f97316', fontWeight: 700 }}>Register</Link></p>
        </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 14, justifyContent: 'center' }}>
          {demoCredentials.map(d => (
            <button key={d.role} onClick={() => fillDemo(d)} style={{
              ...
              borderRadius: 8, color: 'rgba(255,255,255,0.5)', fontSize: 'clamp(12px, 1.0vw, 14px)', fontWeight: 700, cursor: 'pointer',
